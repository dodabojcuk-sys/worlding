...
test("Workspace Layout V1 separates durable project data from transient files", () => {
  ...
  assert.equal(isWorkspaceExportPath(".world-os/runs/provider.tmp"), false);
  assert.equal(isWorkspaceExportPath("continuity/agents/tianyi/sessions/session.000001.jsonl"), true);
  assert.equal(isWorkspaceExportPath("continuity/agents/tianyi/history/sessions/session.000001/revision.json"), true);
  assert.equal(isWorkspaceExportPath("continuity/agents/tianyi/credentials/private.txt"), false);
  assert.equal(isWorkspaceExportPath("world/characters/write.lock"), false);
...
test("package port round-trips durable files without copying cache or accepting conflicts", () => {
  ...
  assert.equal(payload.files.some((file: { path: string }) => file.path === "continuity/agents/tianyi/sessions/session.000001.jsonl"), true);
  assert.equal(JSON.stringify(payload).includes(root), false, "portable packages must not contain absolute workspace paths");
