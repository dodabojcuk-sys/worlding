...
test("event graph R2 keeps the canvas primary and translates implementation terms", () => {
  ...
  assert.match(styles, /event-line-workbench:is\(\[data-projection-mode="line"\], \[data-projection-mode="graph"\], \[data-projection-mode="timeline"\], \[data-projection-mode="participation"\], \[data-projection-mode="perspective"\]\) \.event-line-spine-toolbar/u);
...
test("multi-node productization keeps Unit, active path, overlay, and Tianyi contracts aligned", () => {
  ...
  assert.match(tianyi, /mode === "work" \? <>[\s\S]*<TianyiWorkPanel[\s\S]*<TianyiAgentPanel/u);
  ...
  assert.match(graph, /const predictionSources = predictionRun\?\.sourceSnapshot \?\? \[\]/u);
  assert.match(graph, /const sourceIds = predictionVisible && predictionRun \? new Set\(predictionSources/u);
