...
function fixture() {
  ...
  const unit = { id: "unit.fog", relativeId: "story-units/fog.md", title: "单元 01 · 雾港", summary: "", lifecycle: "active", sourceRefs: [], items: [], linkedEntityIds: events.map((item) => item.id), unresolvedQuestionIds: [], generationConstraints: {}, version: revision, createdAt: new Date(0).toISOString(), updatedAt: new Date(0).toISOString(), source: "markdown" } satisfies StoryUnit;
  return createProjectDirectoryViewModel((key) => zhCN[key], { library, units: [unit], sources: [], workVersionId: "work-r4", pendingCount: 0 });
}
