...
test("Provider settings submit only a one-time credential to the server owner and never expose it", () => {
  ...
  assert.match(settings, /默认对话模型/);
  assert.match(settings, /默认 Embedding 模型/);
  assert.match(settings, /预设建议（未计入已获取）/);
  assert.match(settings, /尚未获取目录/);
  ...
  const saveBody = route.slice(route.indexOf("const saveProvider"), route.indexOf("const discoverModels"));
  assert.doesNotMatch(saveBody, /discoverProviderModels\(/, "saving Provider configuration must make zero catalog calls");
  assert.match(route, /probeProviderEmbedding/);
  assert.match(route, /testProviderConnection/);
