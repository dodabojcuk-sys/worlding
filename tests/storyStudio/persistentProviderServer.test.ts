...
test("Provider Settings persists non-sensitive profile across restart and protects revision/credential boundaries", async () => {
  ...
  try {
    ...
    assert.equal(fakeProvider.calls.completions, 2, "a later author click gets a new operation and a new Provider request");
    ...
    assert.equal(fakeProvider.calls.completions, 3);
