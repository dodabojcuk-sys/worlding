...
function manifest(sessionId: string): TianyiAgentContextManifest {
  return {
...
function fixtureAdapter() {
  ...
  return {
    events,
    adapter: createPiAgentRuntimeAdapter({
      persistence: {
        ...
        async readEvents(input) { return events.filter((event) => event.runId === input.runId && event.projection.projectId === input.projectId && event.projection.sessionId === input.sessionId); }
      },
      async buildContextManifest(input) { return manifest(input.sessionId); },
...
test("Tianyi runtime keeps a recoverable plan, approvals and owner handoff in one adapter", async () => {
  ...
  const started = await fixture.adapter.startRun({ projectId: "project-fixture", sessionId: "session.fixture", task: "检查角色边界", currentPage: "/tianyi", operationId: "operation.start" });
  assert.equal(started.status, "awaiting_author");
  assert.equal(started.plan[0]?.requiredPermission, "author-approval");

  const contextReady = await fixture.adapter.approveStep({ projectId: "project-fixture", sessionId: started.sessionId, runId: started.runId, stepId: started.plan[0]!.stepId, operationId: "operation.approve.context" });
  assert.equal(contextReady.contextManifest?.sessionId, "session.fixture");
  const analyzed = await fixture.adapter.continueRun({ projectId: "project-fixture", sessionId: started.sessionId, runId: started.runId, operationId: "operation.analysis" });
  assert.equal(analyzed.status, "awaiting_author");
  assert.equal(analyzed.candidates[0]?.state, "pending");

  const handedOff = await fixture.adapter.handoffCandidate({ projectId: "project-fixture", sessionId: started.sessionId, runId: started.runId, candidateId: "candidate.fixture.character", operationId: "operation.handoff" });
  assert.equal(handedOff.candidates[0]?.state, "handed-off");
  assert.equal(handedOff.candidates[0]?.ownerReceipt?.id, "proposal.fixture");

  const paused = await fixture.adapter.pauseRun({ projectId: "project-fixture", sessionId: started.sessionId, runId: started.runId, operationId: "operation.pause" });
  assert.equal(paused.status, "paused");
  const resumed = await fixture.adapter.resumeRun({ projectId: "project-fixture", sessionId: started.sessionId, runId: started.runId, operationId: "operation.resume" });
  assert.equal(resumed.status, "completed");

  const recoveredAdapter = fixtureAdapter();
  recoveredAdapter.events.push(...fixture.events);
  const recovered = await recoveredAdapter.adapter.recoverRun({ projectId: "project-fixture", sessionId: "session.fixture", runId: started.runId });
...
test("event-line work runs the bounded read loop without provider or semantic writes", async () => {
  const fixture = fixtureAdapter();
  const started = await fixture.adapter.startRun({ projectId: "project-fixture", sessionId: "session.event", task: "检查明确因果、开放问题和局部节奏", currentPage: "/event-line", operationId: "operation.event.start" });
  const contextReady = await fixture.adapter.approveStep({ projectId: started.projectId, sessionId: started.sessionId, runId: started.runId, stepId: started.plan[0]!.stepId, operationId: "operation.event.approve" });
...
test("Tianyi runtime preserves author control across rejection, steering and cancellation", async () => {
  const fixture = fixtureAdapter();
  const started = await fixture.adapter.startRun({ projectId: "project-fixture", sessionId: "session.control", task: "检查当前上下文", currentPage: "/tianyi", operationId: "operation.control.start" });
  const rejected = await fixture.adapter.rejectStep({ projectId: started.projectId, sessionId: started.sessionId, runId: started.runId, stepId: started.plan[0]!.stepId, reason: "暂不读取来源", operationId: "operation.control.reject" });
  assert.equal(rejected.status, "paused");
  assert.equal(rejected.contextManifest, null);
  const steered = await fixture.adapter.steerRun({ projectId: started.projectId, sessionId: started.sessionId, runId: started.runId, instruction: "只看当前章节", operationId: "operation.control.steer" });
  assert.equal(steered.steering[0]?.instruction, "只看当前章节");
  const cancelled = await fixture.adapter.cancelRun({ projectId: started.projectId, sessionId: started.sessionId, runId: started.runId, reason: "作者停止", operationId: "operation.control.cancel" });
  assert.equal(cancelled.status, "cancelled");
  assert.equal(cancelled.error?.category, "cancelled");
  const duplicateCancel = await fixture.adapter.cancelRun({ projectId: started.projectId, sessionId: started.sessionId, runId: started.runId, reason: "重复停止", operationId: "operation.control.cancel" });
