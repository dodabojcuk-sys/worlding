...
| 页面工具轨 | 页面工具开关 | 固定顺序为工程日志、专家分析、读者鉴赏、语言检查、历史记录、扩展应用 |
...
右侧工具面板栈与天意仅在中央工作区仍有可读宽度的足够宽视口下并列；否则切换为不占布局宽度的覆盖式面板。小屏使用不透明抽屉、全屏层或 Sheet，中央工作区始终优先。

## 面板协议

`apps/story-studio/src/product-shell/right-dock/useDockLayoutState.ts` 只定义瞬时表现协议：`openPanelIds`、`panelOrder`、`panelSizes`、`activeToolId` 与 `isTianyiOpen`。R0.2 实现多面板开关、固定顺序、上下拼接和有边界的鼠标/键盘高度调整；不实现拖拽换边、磁吸、浮动或布局持久化。
