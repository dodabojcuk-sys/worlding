...
历史导航与 MAP-M3、资料 M1 已合入并保留。当前 M2 在原“资料”入口补普通文件 Owner：文件不再伪装成物品或世界对象，稳定文件身份下保存不可变字节修订、文件夹、归档、关联与可恢复批次回执；文本可按明确选段进入 Grounded Context Gate，旧回答按原 SHA 与选段返回。现有 SourceImport、WorldObject、地图和关系 Owner 不迁移。测试使用隔离作品和假 Provider，真实 Provider 0 次；日常 4191/4192 不在本轮切换。
