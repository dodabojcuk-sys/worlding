...
| NUWA-N2B | 在同一角色的允许材料内稳定选择最相关注意力 | Nuwa role context、权限/可知投影、确定性预算选择器 | 实现中 | 先权限，再 must-keep/目标相关性/预算；同输入稳定复现，检查器展示实际入选/排除理由，秘密身份不因排除记录泄漏。 |
