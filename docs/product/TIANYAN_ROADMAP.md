...
| MAP-M4 | 管理多张地图、表达明确空间位置，并把地图图示接到地点资料、局部地图与天意上下文 | `/library?libraryView=map`、`visualDocumentRepository`、World Object Owner、Map Edit Proposal Repository | 点／范围／校准、AI 审阅及地图地点／天意创作起步工程闭环在后续分支本地通过；R2 真实模型案例失败；作者待验 | 已交付管理、跨图端点、人工编辑、受保护提案接受／拒绝／撤销、当前地图精确历史只读，以及点／范围／两点相似变换校准。R4 允许作者把明确图示关联到当前作品的既有地点，查看简洁地点卡、关联或进入局部地图并返回恢复。R5 将地图来源、真实引用和可直接输入的创作意图提到既有天意 Work lane 首屏，支持引用查看／移除、草稿与往返状态保留及诚实的本地成功／失败表达；不自动发送、不新建地图聊天或写入事实。同名图示不会自动造地点，解除关联不删资料或地图。确定性绕行仍只支持作者明确指定的 area 多边形；真实模型结果质量、图片模型、成套历史空间网络和工程包正式导入仍未完成。 |
