...
| 天意／Provider 正式结构化地图请求 | 尚未实现 | 普通天意 Provider 通路当前返回文本回答；M4 按钮是明确的本地确定性假服务，只验证服务端提案链，不证明模型理解或绘图质量；视觉／图片模型未接入 |
...
| MAP-M4 | 管理多张地图、表达明确空间位置，并安全接受人工／AI 共同编辑 | `/library?libraryView=map`、`visualDocumentRepository`、Map Edit Proposal Repository | 点／范围／校准空间编辑本地与隔离浏览器通过；真实模型未验；作者待验 | 已交付管理、跨图端点、人工编辑、受保护提案接受／拒绝／补偿、当前地图精确历史只读，以及点／范围／两点相似变换校准的预览、保存、取消和重开闭环。校准不支持反射、非均匀缩放、仿射、透视、GIS 或真实经纬度；成套历史空间网络、工程包正式导入和普通 Provider 结构化地图请求仍未完成。 |
