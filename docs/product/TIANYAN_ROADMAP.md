...
> 本文是路线图和能力状态的唯一项目内入口。它不替代 `TIANYAN_PRODUCT_CORE.md` 的产品定义，也不以测试通过冒充真实模型或作者体验验收。最后核对分支：`codex/r4-r1-review`；R4 收尾修复分为取消/读快照提交 `97b771a` 和工具栏提交 `25b7ef8`，后续状态以最新提交和运行证据为准。
...
| --- | --- | --- | --- | --- |
| STAB-R4 | 冷启动、目录与世界时间切换可恢复，失败可定位 | Shell runtime、工程目录、EventLine 只读投影 | 本地通过；远端检查待更新；作者待验 | 目录全流程已通过；取消成为终态，旧读响应不得回填写后快照；工具栏低频操作收入固定“更多”；通过后冻结天意/事件线新功能 |
...
| NUWA-N1 | 在受限场景让 2–3 个角色持续行动并可暂停恢复 | RunPack、rehearsal contract、既有 Agent runtime | R4 后的下一主切片；真实模型待验 | 角色稳定身份与知情隔离、有限上下文、暂停/恢复/停止/回放、结果进入既有待确认；不重建 Pi runtime |
