...
| 天意／Provider 正式结构化地图请求 | 后续分支本地与真实模型通过；作者待验 | `codex/map-real-ai-collaboration-r1` 复用 Shell 天意 Work lane、既有 Provider Gateway 与 Map Edit Proposal Repository；只向模型发送明确选择／区域及只读邻近图形，严格工具输出只能形成待审提案。最终隔离链两次发送分别完成道路绕林和区域新增；为入口兼容修复前后各录一轮，本阶段真实发送合计 4 次、无重试。接受／拒绝、重开、陈旧修订、锁层、取消、幂等与补偿冲突均受既有 Owner 保护；图片模型未接入。 |
...
| MAP-M4 | 管理多张地图、表达明确空间位置，并安全接受人工／AI 共同编辑 | `/library?libraryView=map`、`visualDocumentRepository`、Map Edit Proposal Repository | 点／范围／校准与真实文本模型协作在后续分支本地通过；作者待验 | 已交付管理、跨图端点、人工编辑、受保护提案接受／拒绝／补偿、当前地图精确历史只读，以及点／范围／两点相似变换校准。后续真实 AI 分支允许从当前地图所选图形或明确图层区域发起严格结构化文本模型提案；正式写入仍仅在作者接受后经 VisualDocument Owner 完成。校准限制不变；图片模型、成套历史空间网络和工程包正式导入仍未完成。 |
