...
**作者工作台 R2（实现中，作者待验）：**地图集以现有文档派生内容缩略图、上层路径与按需管理；空间视图和分步 A/B 对齐复用单图几何渲染，范围可拖动及逐顶点调整，保存／取消位于同一任务区。三层隔离故事“北湾区域图 → 雾港街区图 → 海关楼室内”覆盖非中心放置、范围调整、相似变换预览、重开、进入绘制及恢复返回；最终工程结果与源码绑定证据索引见既有 M4 工作日志。唯一 VisualDocument Owner、历史只读和受保护提案补偿保持，真实模型未验，作者体验待人工确认；不启动 M5 或切换日常服务。
