...
**地图作者工作台 R3（实现与验收收尾中，作者待验）：**沿用 PR #25 / #24 依赖，在完整地理背景上提供独立紧凑工具栏、按需属性、图层入口、对象/文字/节点直接移动及自动保存重开；新陆水/河路/聚落样式不改写旧对象。AI 审阅与属性协调，固定两端改为显式可选约束，只读参考不意味着不可穿越。使用同一隔离手绘区域地图核对两种桌面尺寸和本地提案，真实 Provider 0 次；R2 真实调用未形成提案、旧底图兼容限制仍保留。最终源码与证据见既有 M4 工作日志。
