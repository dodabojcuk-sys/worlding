...
| REL-F0 | 围绕人物或地点清楚阅读正式关系，并按需展开或回到地图 | 世界入口、Relation Owner、地图观察位置 | 实现中 | 默认只读中心对象的直接已确认关系；可切换图/列表、按类型筛选、逐节点展开及主动全局查看。地图入口携带作品版本、故事观察位置与返回 URL；缺少世界有效时间的关系不伪装为历史关系。浏览不写入 Relation、WorldState 或人物记忆。 |
