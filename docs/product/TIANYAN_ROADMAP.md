...
**作者工作台 R2（本地作者流程通过，作者待验）：**地图集以现有文档派生内容缩略图、上层路径与按需管理；空间视图和分步 A/B 对齐复用单图几何与底图渲染，范围可拖动及逐顶点调整，保存／取消位于同一任务区。三层隔离故事“北湾区域图 → 雾港街区图 → 海关楼室内”覆盖非中心放置、范围调整、相似变换预览、重开、进入绘制及恢复返回。源码 `9341831ea76ce91819810c27aba264a33dcdd24c`（含未保存主导航保护） 的同源连续录像、五张截图和作品身份见既有 M4 工作日志 R2 最终索引；远端完整门禁以 PR #24 最终 head 为准。唯一 VisualDocument Owner、历史只读和受保护提案补偿保持，真实模型未验，作者体验待人工确认；不启动 M5 或切换日常服务。

截至 2026-09-13 09:55 +08:00，M4 继续冻结在既定边界，不启动 M5。`visualDocumentRepository` 仍是唯一地图文档 Owner；`mapEditProposalRepository` 只保存提案与决定／补偿回执。PR #24 原候选 `ba2c34352ed1ad7346669b03d69963b421d8014b` 的两条 `verify` 已完成并通过；坐标校准 R1 沿同一分支和 PR 接续，保留 M4 收尾修复与点／范围定位提交链，最终远端 CI 以本轮 PR head 为准。
...
| 坐标对应／校准 | 本地与隔离浏览器通过；作者待验 | 两个子图／父图对应点确定不反射二维相似变换，公式为 `parent = translate + scale × rotate(child)`；两图都按自身 bounds、x 右／y 下坐标解释，视口 pan/zoom 不进入地图坐标。重合、越界和极端缩放在保存前拒绝；同一父图 placement 保存对应点、解算变换与投影轮廓，管理预览和父图子图叠加消费同一变换。point、range、calibrated 是互斥显式模式，不推导世界事实。 |
