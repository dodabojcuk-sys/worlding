...
| MAP-M4 | 管理多张地图、表达明确空间位置，并安全接受人工／AI 共同编辑 | `/library?libraryView=map`、`visualDocumentRepository`、Map Edit Proposal Repository | 点／范围／校准与 AI 审阅工程闭环在后续分支本地通过；R2 真实模型案例失败；作者待验 | 已交付管理、跨图端点、人工编辑、受保护提案接受／拒绝／撤销、当前地图精确历史只读，以及点／范围／两点相似变换校准。AI 审阅现可在同地图聚焦查看修改前、未保存的修改后和叠加对比，并区分 AI 理由与系统几何检查。确定性绕行只支持作者明确指定的 area 多边形；旧森林装饰笔触无法证明整体占地。正式写入仍仅在作者接受后经 VisualDocument Owner 完成。图片模型、成套历史空间网络和工程包正式导入仍未完成。 |
