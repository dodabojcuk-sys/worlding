...
| 点定位 | 已实现但缺关键验证 | Owner 支持稳定 point placement；正常 UI 仅能把未定位子图初放到父图中心，尚不能继续调整 |
| 范围定位 | 已实现但缺关键验证 | Owner 支持 range，区域建立局部图会生成范围；没有通用作者范围编辑／重开持久化闭环 |
| 坐标对应／校准 | 尚未实现 | 数据合同可保存 calibrated transform，但没有作者校准入口、总览同坐标动态验证或实际校准样例 |
...
隔离证据位于 `data/2026-09-13_天衍地图管理与AI共同编辑M4/closeout-attempt-2/`：地图与作品身份、7 张截图和连续录像对应同一运行；假 Provider 1 次、真实 Provider 0 次。Founder 作者体验仍未验收。日常 4191/4192 在本次只读检查时均无监听，本轮未启动、停止或切换服务，正式数据未迁移。
...
| MAP-M4 | 管理多张地图、表达明确空间位置，并安全接受人工／AI 共同编辑 | `/library?libraryView=map`、`visualDocumentRepository`、Map Edit Proposal Repository | 阶段冻结候选；本地定向与隔离浏览器通过；真实模型未验；作者待验 | 已交付管理、跨图端点、人工编辑、受保护提案接受／拒绝／补偿和当前地图精确历史只读。点初放已有但不可继续调整；范围编辑、坐标校准、成套历史空间网络、工程包正式导入和普通 Provider 结构化地图请求未完成。后续唯一优先动作是先经作者确认决定是否以“可调整点／范围定位与坐标校准”为独立切片；不得在 M4 收尾中继续扩建。 |
