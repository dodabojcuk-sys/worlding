...
import { BookOpen, ChevronDown, ClipboardList, Search, Settings, UserRound, X } from "lucide-react";
...
export function NuwaUnifiedDirectory(props: {
  ...
  return <aside className="project-directory-panel nuwa-unified-directory" aria-label="统一目录" data-testid="nuwa-unified-directory">
    ...
    <div className="nuwa-directory-quick"><button type="button" onClick={props.onSearch}><Search aria-hidden="true" />搜索当前项目</button></div>
    <details className="nuwa-directory-spaces" open={spacesOpen} onToggle={(event) => setSpacesOpen(event.currentTarget.open)}><summary>项目空间 · {space === "nuwa" ? "女娲" : "事件线"}</summary><nav aria-label="项目内空间">{STORY_STUDIO_SHELL_NAVIGATION_REGISTRY.filter((item) => item.enabled).map((item) => <button key={item.id} type="button" aria-current={item.id === space ? "page" : undefined} onClick={() => props.onNavigate(item)}>{t(item.labelKey as TranslationKey)}</button>)}</nav></details>
    {space === "nuwa" ? <div className="nuwa-directory-views" role="tablist" aria-label="女娲目录视图"><button type="button" role="tab" aria-selected={view === "conversations"} onClick={() => selectView("conversations")}>对话与排演</button><button type="button" role="tab" aria-selected={view === "structure"} onClick={() => selectView("structure")}>故事结构</button></div> : <div className="nuwa-directory-views"><strong>故事结构</strong></div>}
    {space === "nuwa" ? <div className="nuwa-directory-content" hidden={view !== "conversations"}><ProjectConversationPanel key={projectId ?? "none"} runtime={props.runtime} currentProjectOnly onDirectory={() => selectView("structure")} onClose={props.onClose} /></div> : null}
    <div className="nuwa-directory-content is-structure" ref={structureRef} hidden={space === "nuwa" && view !== "structure"} onScroll={(event) => { if (projectId) try { window.sessionStorage.setItem(`tianyan-nuwa-structure-scroll:${projectId}`, String(event.currentTarget.scrollTop)); } catch { /* navigation remains usable */ } }}>
