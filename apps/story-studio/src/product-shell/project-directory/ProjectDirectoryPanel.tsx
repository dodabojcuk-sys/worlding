...
export function ProjectDirectoryPanel(props: { runtime: TianyanShellRuntimeState; project: Parameters<typeof useProjectDirectoryProjection>[0]; mode: ProjectDirectoryMode; onClose(): void; onModeChange(mode: ProjectDirectoryMode): void; onOpenPendingReview(target: StoryIntakeReviewTarget | null): void; onOpenRelationReview(): void; onNavigate(node: ProjectDirectoryNode): void; onOpenReference(reference: ProjectDirectoryStableReference): void; selectedObjectId: string | null; onCreateProject(title: string): Promise<void> }) {
  ...
  const storyIntakeBatches = Array.from(new Map((state.pending?.items ?? [])
    .filter((item) => item.pending && item.storyIntakeTarget)
    .map((item) => [item.sourceBatch.id, { id: item.sourceBatch.id, label: item.sourceBatch.label, target: item.storyIntakeTarget!, count: 0 }])).values())
    .map((batch) => ({ ...batch, count: (state.pending?.items ?? []).filter((item) => item.pending && item.sourceBatch.id === batch.id).length }));
  ...
  return <aside className="project-directory-panel" aria-label={t("panel.projectDirectory")} data-story-fact-owner="false">
    ...
    {props.mode === "pending" && <section className="pending-review-directory-summary" aria-label={t("directory.pendingLabel")} data-story-fact-owner="false">
      <PendingReviewEntry count={state.pending?.pendingCount ?? 0} onOpen={() => props.onOpenPendingReview(state.pending?.storyIntakeTargets[0] ?? null)} />
      <p>{t("directory.pendingEntryDescription")}</p>
      <dl aria-label={t("directory.pendingCategoryLabel")}>
        <div><dt>{t("directory.pendingCategory.source")}</dt><dd>{state.pending?.categoryCounts.source ?? 0}</dd></div>
        <div><dt>{t("directory.pendingCategory.candidate")}</dt><dd>{state.pending?.categoryCounts.candidate ?? 0}</dd></div>
        <div><dt>{t("directory.pendingCategory.decision")}</dt><dd>{state.pending?.categoryCounts.decision ?? 0}</dd></div>
        <div><dt>{t("directory.pendingCategory.adoption")}</dt><dd>{state.pending?.categoryCounts.adoption ?? 0}</dd></div>
        <div><dt>{t("directory.pendingCategory.processed")}</dt><dd>{state.pending?.categoryCounts.processed ?? 0}</dd></div>
      </dl>
      {storyIntakeBatches.length > 0 && <ul className="pending-review-story-intake-batches" aria-label="Story Intake source batches">
        {storyIntakeBatches.map((batch) => <li key={batch.id} data-pending-story-intake-batch="true"><button type="button" onClick={() => props.onOpenPendingReview(batch.target)}><span>{batch.label}</span><strong>{t("directory.pendingItemCount").replace("{count}", String(batch.count))}</strong></button></li>)}
