import { BookOpen, ChevronLeft, ChevronRight, Link2, UserRound, X } from "lucide-react";
...
export function CharacterInspectorCard(props: { record: CharacterDirectoryRecord; onClose(): void }) {
  ...
  return <aside className="character-inspector" aria-label={t("character.inspector")} aria-expanded={expanded} data-expanded={expanded} data-testid="character-inspector">
    <header><div className="character-avatar">{object.card.portrait ? <img src={object.card.portrait.assetRef} alt="" /> : <UserRound aria-hidden="true" />}</div><div className="character-inspector-identity"><h2>{object.title}</h2><div><span>{role}</span><b>{object.status === "archived" ? t("character.archived") : t("character.confirmed")}</b></div></div><div className="character-inspector-header-actions"><button type="button" onClick={() => setExpanded((value) => !value)} aria-label={expanded ? t("character.collapseInspector") : t("character.expandInspector")} aria-expanded={expanded} title={expanded ? t("character.collapseInspector") : t("character.expandInspector")}>{expanded ? <ChevronRight aria-hidden="true" /> : <ChevronLeft aria-hidden="true" />}</button><button ref={closeRef} type="button" onClick={props.onClose} aria-label={t("common.close")}><X aria-hidden="true" /></button></div></header>
