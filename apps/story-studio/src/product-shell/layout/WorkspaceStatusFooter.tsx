...
export function WorkspaceStatusFooter(props: {
  ...
  tianyiDocked: boolean;
  onTasks(): void;
  onFunctionHistory(): void;
  onTianyi(): void;
  onPinTianyi(): void;
}) {
  return <footer className="shell-workspace-status" aria-label="工作区状态">
    ...
    {props.currentRunId ? <button type="button" onClick={props.onFunctionHistory}>功能历史</button> : null}
    <div className="shell-status-assistant"><button type="button" data-panel-toggle="tianyi-agent" onClick={props.onTianyi} aria-pressed={props.tianyiOpen}><Sparkles aria-hidden="true" />{props.tianyiOpen ? "关闭天意" : "召唤天意"}</button>{props.tianyiOpen ? <button type="button" className="shell-status-pin" onClick={props.onPinTianyi} aria-pressed={props.tianyiPinned} title={props.tianyiPinned ? props.tianyiDocked ? "取消固定右侧天意" : "宽度不足，暂以浮窗显示；固定偏好仍保留。点击可取消固定偏好。" : "固定到右侧"}><Pin aria-hidden="true" /><span>{props.tianyiPinned ? props.tianyiDocked ? "已固定" : "暂作浮窗" : "固定"}</span></button> : null}</div>
  </footer>;
}
