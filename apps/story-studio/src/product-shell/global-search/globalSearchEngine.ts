...
function searchRoutes(query: string, context: GlobalSearchContext): GlobalSearchResult[] {
  ...
  for (const destination of STORY_STUDIO_SHELL_NAVIGATION_REGISTRY) {
    if (!destination.enabled) continue;
      const match = matchText(query, [destination.displayName, destination.id, destination.route]);
      if (!match) continue;
      results.push({
...
function searchNavigationCommands(query: string, context: GlobalSearchContext): GlobalSearchResult[] {
  ...
  for (const destination of STORY_STUDIO_SHELL_NAVIGATION_REGISTRY) {
    if (!destination.enabled) continue;
      const match = matchText(query, ["前往", "navigate", destination.displayName, destination.id, destination.route]);
      if (!match) continue;
      results.push({
        ...
        stableReference: contextReference(context),
        target: { route: destination.route },
        matchReason: query ? match.reason : "导航命令"
      });
  }
  return results;
}
...
function searchObjects(project: GlobalSearchProjectReadModel, query: string, scope: GlobalSearchScope): GlobalSearchResult[] {
  ...
  for (const object of project.objects) {
      ...
      const match = matchText(query, [object.title, ...object.aliases, ...object.tags, objectTypeLabels[object.type], object.type]);
      if (!match) continue;
...
function searchSources(project: GlobalSearchProjectReadModel, query: string): GlobalSearchResult[] {
  ...
  for (const source of project.sources) {
      const match = matchText(query, [source.title, source.filename, source.mode, "资料", "来源"]);
      if (!match) continue;
      results.push({
...
function matchText(query: string, candidates: readonly string[]): { reason: string } | null {
  if (!query) return { reason: "浏览全部" };
  const normalizedCandidates = candidates.map(normalize);
  if (normalizedCandidates.some((candidate) => candidate === query)) return { reason: "标题完全匹配" };
  if (normalizedCandidates.some((candidate) => candidate.includes(query))) return { reason: "名称或关键词匹配" };
  return null;
}
