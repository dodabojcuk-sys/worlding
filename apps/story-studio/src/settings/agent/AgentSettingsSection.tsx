...
export function AgentSettingsSection(props: {
  ...
  const credentialBackendDisabled = credential?.backend === "disabled";
  const credentialActionBlocked = credentialBackendDisabled || providerBusy || props.busy || !props.onSaveProviderProfile;
  ...
  return <section id="settings-agent-overview" className="settings-card agent-settings-section" aria-labelledby="agent-settings-title" data-agent-runtime="pi">
    ...
    <form id="settings-agent-provider" ref={providerForm} className="agent-provider-profile" onSubmit={saveProvider} key={`${props.status?.profile.revision ?? "initial"}:${providerId}`}>
      ...
      <dl>
        ...
        <div><dt>凭据</dt><dd className="agent-provider-credential-status">{selectedProviderMatches && credential?.configured && <LockKeyhole aria-hidden="true" />}{providerPreset?.credentialRequired === false ? "本地运行时无需凭据" : selectedProviderMatches && credential?.configured ? "已锁定保存" : selectedProviderMatches && credentialBackendDisabled ? "环境禁用：部署未启用凭据后端" : selectedProviderMatches ? "未配置" : "保存后读取该实例凭据状态"}</dd></div>
      ...
      <div className="agent-provider-secret-field">
        {credentialBackendDisabled && <p role="alert" className="agent-provider-disabled-note">当前审阅部署未启用 Provider 凭据后端（TIANYAN_CREDENTIAL_BACKEND=DISABLED）。在这里输入或保存密钥都不会成功；请先联系运维启用生产凭据后端，再回到本页配置模型。</p>}
        {selectedProviderMatches && credential?.configured && !replaceCredential ? <><label htmlFor="saved-provider-api-key">API Key</label><div className="agent-provider-secret-control">{revealedCredential ? <input id="saved-provider-api-key" value={revealedCredential} readOnly autoComplete="off" aria-label="已保存的 API Key（临时显示）" /> : <output id="saved-provider-api-key">已保存 · ••••••••</output>}<button type="button" disabled={providerBusy || props.busy || !props.onRevealProviderCredential} onClick={revealCredential}>{revealedCredential ? "隐藏已保存密钥" : "显示已保存密钥"}</button><button type="button" disabled={credentialActionBlocked} onClick={() => { clearRevealedCredential(); setReplaceCredential(true); }}>更换</button></div><small className="agent-provider-secret-note"><LockKeyhole aria-hidden="true" />已保存。显示只保留在当前组件内，离开、切换实例、再次隐藏或 20 秒后清除。</small></> : <><label htmlFor="provider-api-key">{replaceCredential ? "新的 API Key（可选）" : "API Key"}</label><div className="agent-provider-secret-control"><input id="provider-api-key" ref={credentialInput} name="apiKey" type={showCredentialDraft ? "text" : "password"} autoComplete="new-password" placeholder={replaceCredential ? "输入新 Key 后保存以替换" : "输入 API Key"} disabled={credentialActionBlocked} /><button type="button" aria-label={showCredentialDraft ? "隐藏本次输入的 API Key" : "显示本次输入的 API Key"} title={showCredentialDraft ? "隐藏本次输入" : "显示本次输入"} disabled={credentialActionBlocked} onClick={() => setShowCredentialDraft((current) => !current)}>{showCredentialDraft ? <EyeOff aria-hidden="true" /> : <Eye aria-hidden="true" />}</button>{replaceCredential && <button type="button" onClick={() => setReplaceCredential(false)}>取消更换</button>}</div><small className="agent-provider-secret-note"><LockKeyhole aria-hidden="true" />留空保存其他字段会保留原 Key；保存失败也不会清除已保存凭据。</small></>}
      </div>
      ...
      <div className="agent-provider-actions">
        <button type="submit" disabled={credentialActionBlocked} title={credentialBackendDisabled ? "当前部署未启用凭据后端；无法保存凭据" : undefined}>{providerAction === "save" ? "正在保存…" : "保存 Provider 配置"}</button>
        <button type="button" className={networkReady && catalog?.status === "never_fetched" ? "settings-primary-action" : undefined} disabled={credentialActionBlocked || !networkReady || !props.onDiscoverProviderModels} onClick={discoverModels}>{catalogOperation.phase === "running" ? "正在获取模型…" : endpointEntries.length ? "重新获取模型" : "获取模型"}</button>
        <button type="button" disabled={credentialActionBlocked || !networkReady || !props.onTestProviderConnection} onClick={testConnection}>{connectionOperation.phase === "running" ? "正在测试连接…" : "测试连接"}</button>
        <button type="button" disabled={credentialActionBlocked || !props.onSaveProviderProfile || !props.onTestProviderConnection} onClick={saveAndTest}>{providerAction === "connection" ? "正在保存并测试…" : "保存并测试"}</button>
        <button type="button" disabled={credentialActionBlocked || !networkReady || !props.onProbeEmbedding} onClick={probeEmbedding}>验证 Embedding</button>
