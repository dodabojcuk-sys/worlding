...
export function MapM1Workspace(props: { runtime: TianyanShellRuntimeState }) {
  ...
  const cancelSpatialEdit = () => { setSpatialDraft(null); setMessage("已取消空间定位编辑；已保存位置没有改变。"); };
  ...
  const startSpatialDrag = (event: PointerEvent<HTMLButtonElement>, placement: MapPlacement) => { if (!spatialDraft || spatialDraft.placement.id !== placement.id) return; event.preventDefault(); event.stopPropagation(); event.currentTarget.setPointerCapture(event.pointerId); spatialDrag.current = { pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, placement: structuredClone(placement) }; };
  ...
  return <main className="shell-workspace map-workbench-shell" aria-label="地点地图">
    <section className={`map-workbench ${editingRegionObjectId ? "is-boundary-editing" : ""}`} data-testid="map-m2-workspace">
      ...
      {!maps.length ? <section className="map-workbench-empty"><h1>建立第一张作者地图</h1><p>选择轻量起点后即可绘制；所有初始图形都只是图示，不会改变地点、关系或角色知情。</p><label>地图起点<select aria-label="地图起点" value={templateChoice} onChange={(event) => setTemplateChoice(event.target.value as MapContent["template"])}><option value="geography">地理区域</option><option value="starfield">星域</option><option value="building">建筑平面</option><option value="blank">空白画布</option></select></label><button type="button" aria-label="建立地点示意图" className="primary-action" disabled={busy} onClick={create}>建立地图</button></section> : !map ? <section className="map-manager" aria-label="地图管理">
        ...
        {managerView === "list" ? <div className="map-manager-list">{searchableMaps.map((item) => { const broken = item.content.connections.filter((connection) => !maps.some((candidate) => candidate.id === connection.from.mapId) || !maps.some((candidate) => candidate.id === connection.to.mapId)).length + item.content.entrances.filter((entrance) => !maps.some((candidate) => candidate.id === entrance.targetMapId)).length; const placements = maps.flatMap((parent) => parent.content.placements.filter((placement) => placement.childMapId === item.id).map((placement) => ({ parent, placement }))); return <article key={item.id} data-archived={item.content.lifecycle.archived}><div className="map-manager-thumbnail">{item.content.backgrounds.length ? <img src={visualAssetUrl(projectId, item.content.backgrounds[0]!.assetPath)} alt="" /> : <MapPin aria-hidden="true" />}</div><div><h2>{item.title}</h2><p>{item.content.scopeObjectId ? locations.find((location) => location.id === item.content.scopeObjectId)?.title ?? "关联地点已失效" : "未关联地点"} · 修订 {item.revision}</p><small>{placements.length ? placements.map(({ parent, placement }) => `${parent.title} · ${placement.kind === "point" ? "点定位" : placement.kind === "range" ? "范围定位" : "已校准"}`).join("；") : "尚未定位"}{broken ? ` · ${broken} 条失效连接` : ""}</small><time>{item.updatedAt ? new Date(item.updatedAt).toLocaleString() : "旧地图 · 更新时间未知"}</time></div><nav><button type="button" onClick={() => navigateToMap(item.id)}>打开</button><button type="button" onClick={() => void duplicateMap(item)}><Copy aria-hidden="true" />复制</button><button type="button" onClick={() => void setMapArchived(item, !item.content.lifecycle.archived)}>{item.content.lifecycle.archived ? <><RotateCcw aria-hidden="true" />恢复</> : <><Archive aria-hidden="true" />归档</>}</button>{placements[0] ? <button type="button" onClick={() => { setManagerView("spatial"); setManagerParentId(placements[0]!.parent.id); setSpatialDraft(null); }}>父图定位</button> : null}</nav></article>; })}</div> : <div className="map-manager-spatial"><label>父地图<select aria-label="空间总览父地图" value={managerParent?.id ?? ""} onChange={(event) => { setManagerParentId(event.target.value); setSpatialDraft(null); }}>{maps.filter((item) => !item.content.lifecycle.archived).map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label>{managerParent ? <div className="map-manager-parent-canvas" aria-label={`${managerParent.title}空间定位画布`} onPointerMove={dragSpatialPlacement} onPointerUp={endSpatialDrag} onPointerCancel={endSpatialDrag}><strong>{managerParent.title}</strong><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">{managerParent.content.placements.map((saved) => spatialDraft?.parentMapId === managerParent.id && spatialDraft.placement.id === saved.id ? spatialDraft.placement : saved).filter((placement) => placement.kind !== "point").map((placement) => <polygon key={placement.id} className={spatialDraft?.placement.id === placement.id ? "is-draft" : ""} points={placement.bounds.map((point) => `${point.x},${point.y}`).join(" ")} />)}{spatialDraft?.parentMapId === managerParent.id && !managerParent.content.placements.some((item) => item.id === spatialDraft.placement.id) && spatialDraft.placement.kind !== "point" ? <polygon className="is-draft" points={spatialDraft.placement.bounds.map((point) => `${point.x},${point.y}`).join(" ")} /> : null}</svg>{managerParent.content.placements.map((saved) => { const placement = spatialDraft?.parentMapId === managerParent.id && spatialDraft.placement.id === saved.id ? spatialDraft.placement : saved; const child = maps.find((item) => item.id === placement.childMapId); if (!child) return null; const point = placementAnchor(placement); return <button key={placement.id} type="button" className={spatialDraft?.placement.id === placement.id ? "is-editing" : ""} style={{ left: `${point.x}%`, top: `${point.y}%` }} aria-label={`编辑 ${child.title} 的${placement.kind === "point" ? "点定位" : "范围定位"}`} onClick={() => beginSpatialEdit(managerParent, placement)} onPointerDown={(event) => startSpatialDrag(event, placement)}>{child.title}<small>{placement.kind === "point" ? "点定位" : placement.kind === "range" ? `范围 · ${placement.bounds.length} 点` : "已校准"}</small></button>; })}{spatialDraft?.parentMapId === managerParent.id && !managerParent.content.placements.some((item) => item.id === spatialDraft.placement.id) ? (() => { const child = maps.find((item) => item.id === spatialDraft.placement.childMapId); const point = placementAnchor(spatialDraft.placement); return child ? <button type="button" className="is-editing" style={{ left: `${point.x}%`, top: `${point.y}%` }} aria-label={`编辑 ${child.title} 的范围定位`} onPointerDown={(event) => startSpatialDrag(event, spatialDraft.placement)}>{child.title}<small>新范围 · {spatialDraft.placement.bounds.length} 点</small></button> : null; })() : null}</div> : null}<aside><h2>{spatialDraft ? "定位编辑" : "尚未定位"}</h2>{spatialDraft && managerParent ? (() => { const child = maps.find((item) => item.id === spatialDraft.placement.childMapId); const anchor = placementAnchor(spatialDraft.placement); return <section className="map-spatial-editor" aria-label="空间定位编辑器"><strong>{child?.title ?? "子地图"}</strong><small>父图：{managerParent.title} · {spatialDraft.placement.kind === "point" ? "点定位" : `范围定位（保留 ${spatialDraft.placement.bounds.length} 个顶点）`}</small><p>画布和数值当前是临时预览；拖动结束不会自动保存。</p><div><label>水平位置<input aria-label="定位水平位置" type="number" min="0" max="100" step="0.1" value={Number(anchor.x.toFixed(2))} onChange={(event) => updateSpatialDraft(movePlacement(spatialDraft.placement, { ...anchor, x: Number(event.target.value) }))} /></label><label>垂直位置<input aria-label="定位垂直位置" type="number" min="0" max="100" step="0.1" value={Number(anchor.y.toFixed(2))} onChange={(event) => updateSpatialDraft(movePlacement(spatialDraft.placement, { ...anchor, y: Number(event.target.value) }))} /></label></div>{spatialDraft.placement.kind !== "point" ? <><label>整体大小（相对当前保存状态）<input aria-label="范围整体大小" type="range" min="25" max="200" step="1" defaultValue="100" onChange={(event) => updateSpatialDraft(scalePlacementBounds(spatialDraft.savedPlacement, Number(event.target.value)))} /></label><details><summary>逐点调整（{spatialDraft.placement.bounds.length}）</summary>{spatialDraft.placement.bounds.map((point, index) => <div key={index}><label>点 {index + 1} X<input aria-label={`范围点 ${index + 1} X`} type="number" min="0" max="100" step="0.1" value={Number(point.x.toFixed(2))} onChange={(event) => updateSpatialDraft({ ...spatialDraft.placement, bounds: spatialDraft.placement.bounds.map((item, pointIndex) => pointIndex === index ? clampMapPoint({ ...item, x: Number(event.target.value) }) : item) })} /></label><label>Y<input aria-label={`范围点 ${index + 1} Y`} type="number" min="0" max="100" step="0.1" value={Number(point.y.toFixed(2))} onChange={(event) => updateSpatialDraft({ ...spatialDraft.placement, bounds: spatialDraft.placement.bounds.map((item, pointIndex) => pointIndex === index ? clampMapPoint({ ...item, y: Number(event.target.value) }) : item) })} /></label></div>)}</details></> : null}<div className="map-spatial-editor-actions"><button type="button" onClick={cancelSpatialEdit} disabled={busy}>取消</button><button type="button" className="primary-action" onClick={saveSpatialEdit} disabled={busy}>保存定位</button></div></section>; })() : unlocatedMaps.length ? unlocatedMaps.map((item) => <div key={item.id}><span>{item.title}</span><span><button type="button" disabled={!managerParent || busy} onClick={() => managerParent && placeChildMap(managerParent, item)}>点定位到中心</button><button type="button" disabled={!managerParent || busy} onClick={() => managerParent && beginNewRange(managerParent, item)}>建立范围</button></span></div>) : <p>当前可用地图都已有明确放置。点击画布上的子地图即可调整。</p>}</aside></div>}
      </section> : <>
