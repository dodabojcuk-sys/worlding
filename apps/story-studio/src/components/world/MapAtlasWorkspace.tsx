...
export function MapAtlasWorkspace(props: {
  ...
  return <section className="map-manager map-atlas" aria-label="地图管理">
    ...
    {props.view==="list" ? <div className="map-manager-list">{[...props.maps].sort((a,b)=>(b.updatedAt??"").localeCompare(a.updatedAt??"")).filter((m)=>(archived||!m.content.lifecycle.archived)&&m.title.includes(search)).map((m)=>{
      ...
      return <article key={m.id} data-archived={m.content.lifecycle.archived}><div className="map-manager-thumbnail"><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label={m.title+"内容缩略图"}><MapReadOnlyContent projectId={props.projectId} content={m.content} compact/></svg></div><div><h2>{m.title}</h2><small>{parents.length ? "上层："+parents.map((p)=>p.title).join(" · ") : "独立地图 · 可作为空间总览起点"}</small><p>{m.content.drawings.length} 个图示 · {m.content.placements.length} 张子图{m.content.lifecycle.archived?" · 已归档":""}</p></div><nav><button onClick={()=>props.onOpen(m.id)}>打开</button><button onClick={()=>{chooseParent(m.id);props.onView("spatial");}}>空间视图</button>{parents[0]?<button onClick={()=>{chooseParent(parents[0]!.id);setSelected(parents[0]!.content.placements.find((p)=>p.childMapId===m.id)!.id);props.onView("spatial");}}>父图定位</button>:null}<details><summary>更多</summary><button onClick={()=>props.onDuplicate(m)}>复制</button><button onClick={()=>props.onArchive(m,!m.content.lifecycle.archived)}>{m.content.lifecycle.archived?"恢复":"归档"}</button><small>修订 {m.revision}</small><time>{m.updatedAt?new Date(m.updatedAt).toLocaleString():"时间未知"}</time></details></nav></article>;
