...
export function FocusedRelationsWorkspace(props: { runtime: TianyanShellRuntimeState }) {
  ...
  const [versionResolutionSettled, setVersionResolutionSettled] = useState(false);
  const observedAt = params.get("mapObservedAt");

  useEffect(() => {
    if (!projectId || workVersionId) { setVersionResolutionSettled(Boolean(workVersionId)); return; }
    setVersionResolutionSettled(false);
    const timer = window.setTimeout(() => setVersionResolutionSettled(true), 700);
  ...
  const writeRoute = (next: Partial<{ center: string; mode: ViewMode; type: string; query: string; all: boolean; expanded: ReadonlySet<string>; viewport: GraphViewport; selection: GraphSelection }>) => {
  ...
  const selectType = (next: string) => { setTypeFilter(next); setSelection(null); setFitNonce((value) => value + 1); updateRoute({ type: next, selection: null }); };
  const updateObjectSearch = (next: string) => { setObjectSearch(next); updateRoute({ query: next }); };
  const selectScope = (next: boolean) => { setShowAll(next); setSelection(null); setFitNonce((value) => value + 1); updateRoute({ all: next, selection: null }); };
  ...
  if (!workVersionId && !versionResolutionSettled) return <main className="shell-workspace focused-relations-shell"><MaterialsSectionNavigation current="relations" /><section className="focused-relations" aria-busy="true"><p>正在恢复当前作品与关系现场……</p></section></main>;
  ...
  return <section className={`focused-relations-graph ${props.selection ? "has-selection" : ""}`} aria-label="对象关系图">
    <div ref={canvasRef} className="focused-relations-canvas" data-testid="focused-relations-canvas" tabIndex={0} aria-label="关系连线画布" onWheel={(event) => { event.preventDefault(); props.onViewport({ ...props.viewport, scale: props.viewport.scale + (event.deltaY < 0 ? .1 : -.1) }); }} onPointerDown={(event) => { if (!(event.target instanceof Element) || event.target.closest("button")) return; event.currentTarget.setPointerCapture(event.pointerId); setDragStart({ x: event.clientX, y: event.clientY }); }} onPointerMove={(event) => { if (!dragStart) return; move({ x: event.clientX - dragStart.x, y: event.clientY - dragStart.y }); setDragStart({ x: event.clientX, y: event.clientY }); }} onPointerUp={() => setDragStart(null)} onKeyDown={(event) => { const step = 24; if (event.key === "ArrowLeft") { event.preventDefault(); move({ x: -step, y: 0 }); } if (event.key === "ArrowRight") { event.preventDefault(); move({ x: step, y: 0 }); } if (event.key === "ArrowUp") { event.preventDefault(); move({ x: 0, y: -step }); } if (event.key === "ArrowDown") { event.preventDefault(); move({ x: 0, y: step }); } if (event.key === "+" || event.key === "=") { event.preventDefault(); props.onViewport({ ...props.viewport, scale: props.viewport.scale + .1 }); } if (event.key === "-") { event.preventDefault(); props.onViewport({ ...props.viewport, scale: props.viewport.scale - .1 }); } if (event.key === "0") { event.preventDefault(); props.onRequestFit(); } }}>
      ...
      <div className="focused-relations-canvas-world" style={{ transform: `translate(${props.viewport.x}px, ${props.viewport.y}px) scale(${props.viewport.scale})` }}>
        <svg className="focused-relations-edges" viewBox="0 0 960 560" aria-hidden="true"><defs><marker id="relation-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" /></marker></defs>{groups.map((group) => { const source = positions.get(group.sourceId); const target = positions.get(group.targetId); if (!source || !target) return null; const directions = new Set(group.relations.map((relation) => relation.direction)); const edge = edgeEndpoints(source, target); const curve = curvedEdge(edge, group.key); const isSelected = props.selection?.kind === "edge" && props.selection.id === group.key; return <path key={group.key} className={isSelected ? "is-selected" : undefined} d={curve.path} markerStart={directions.has("reverse") || directions.has("both") ? "url(#relation-arrow)" : undefined} markerEnd={directions.has("forward") || directions.has("both") ? "url(#relation-arrow)" : undefined} />; })}</svg>
        {groups.map((group) => { const source = positions.get(group.sourceId); const target = positions.get(group.targetId); if (!source || !target) return null; const edge = edgeEndpoints(source, target); const curve = curvedEdge(edge, group.key); return <button key={group.key} type="button" className="focused-relations-edge-label" aria-pressed={props.selection?.kind === "edge" && props.selection.id === group.key} style={{ left: `${curve.label.x}px`, top: `${curve.label.y}px` }} onClick={() => props.onSelection({ kind: "edge", id: group.key })}>{relationGroupLabel(group.relations)}</button>; })}
...
function curvedEdge(edge: ReturnType<typeof edgeEndpoints>, key: string): { path: string; label: Point } { const dx = edge.x2 - edge.x1; const dy = edge.y2 - edge.y1; const length = Math.max(1, Math.hypot(dx, dy)); const sign = [...key].reduce((sum, value) => sum + value.codePointAt(0)!, 0) % 2 ? 1 : -1; const offset = Math.min(34, Math.max(16, length * .08)) * sign; const nx = -dy / length; const ny = dx / length; const cx = (edge.x1 + edge.x2) / 2 + nx * offset; const cy = (edge.y1 + edge.y2) / 2 + ny * offset; return { path: `M ${edge.x1} ${edge.y1} Q ${cx} ${cy} ${edge.x2} ${edge.y2}`, label: { x: (edge.x1 + 2 * cx + edge.x2) / 4, y: (edge.y1 + 2 * cy + edge.y2) / 4 } }; }
...
function uniqueEvidenceRelations(relations: readonly RelationReadProjectionR0[]): RelationReadProjectionR0[] { const seen = new Set<string>(); return relations.filter((relation) => { const reference = exactEventReference(relation); const key = reference ? `${reference.eventId}@${reference.revision}` : `missing:${relation.relationId}`; if (seen.has(key)) return false; seen.add(key); return true; }); }
