...
export function FocusedRelationsWorkspace(props: { runtime: TianyanShellRuntimeState }) {
  ...
  const selectCenter = (id: string) => { const next = new Set<string>(); setCenterId(id); setExpanded(next); setSelection({ kind: "node", id }); setSelectedRelationId(""); setCreateFormOpen(false); setFitNonce((value) => value + 1); updateRoute({ center: id, expanded: next, selection: { kind: "node", id }, selectedRelationId: "" }); };
