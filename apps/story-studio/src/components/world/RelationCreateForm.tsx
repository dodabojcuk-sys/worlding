...
/**
 * 人工人物关系表单（右侧抽屉面板）：
 * 双方人物按稳定 ID 选择（同名以 ID 尾号区分），关系名称复用既有 RelationType，
 * 依据必须是当前版本内经作者确认链验证的正式事件；
 * 建立后先成为候选，作者显式确认才写入正式关系。
 */
export function RelationCreateForm(props: {
  ...
  if (!evidenceEventId) missing.push("依据事件");
  ...
  const submitCandidate = () =>
    run(async () => {
      ...
      if (!evidenceEvent) { setError("请选择依据事件；人物关系需要已确认事件作为可追溯依据。当前版本还没有可用事件时，请先到事件线用“常规创作”确认。"); return; }
      ...
      setStatus(duplicates.suggestions.length ? `已建立候选；注意：同一对人物已有 ${duplicates.suggestions.length} 条同类型关系，新关系是额外一条，不会覆盖它们。` : "");
    });
  ...
  return <aside className="focused-relations-create" aria-label="人工建立关系">
    ...
    <p className="focused-relations-create-intro">双方按稳定 ID 绑定（同名以 ID 尾号区分）；依据必须是本版本内经作者确认链验证的正式事件。</p>
    {candidate ? <div className="focused-relations-candidate" role="status">
      ...
      <p className="focused-relations-candidate-evidence">依据事件：{evidenceEvent?.title ?? candidate.relationLabelSnapshot}{evidenceEvent ? `（修订 ${evidenceEvent.revisionToken.slice(0, 8)}）` : ""}</p>
      ...
      <div className="create-group">
        ...
        <label className="create-field"><span>来源人物</span>
          <select aria-label="关系来源人物" value={sourceId} disabled={busy} onChange={(event) => setSourceId(event.target.value)}>
            ...
            {characters.map((item) => <option key={item.id} value={item.id}>{item.title}（{item.id.slice(-6)}）</option>)}
          </select>
        </label>
      ...
      <div className="create-group">
        <p className="create-group-title">依据</p>
        <label className="create-field"><span>依据事件（必选）</span>
          <select aria-label="关系依据事件" value={evidenceEventId} disabled={busy} onChange={(event) => setEvidenceEventId(event.target.value)}>
            ...
            {usableEvents.map((event) => <option key={event.id} value={event.id}>{event.title}（修订 {event.revisionToken.slice(0, 8)}）</option>)}
          </select>
        </label>
        {!usableEvents.length ? <p className="create-field-error" role="alert">还没有可用的已确认事件——请先到事件线用“常规创作”确认一个事件，关系需要可追溯依据。</p> : null}
      </div>
      {sourceId && targetId && chosenTypeLabel ? <div className="focused-relations-create-preview" role="status">
        ...
        {evidenceEvent ? <p className="focused-relations-create-preview-evidence">依据事件：{evidenceEvent.title}</p> : null}
