...
export function CreationSourceWorkspace(props: { runtime: TianyanShellRuntimeState }) {
  ...
  return <main className="shell-workspace shell-workspace-writing" aria-label="创作"><section className="creation-source-workspace" data-testid="creation-source-workspace" data-package-id={state?.package?.id}>
    ...
    {!state.root && !sourceRequestBlocker ? <section className="creation-source-missing" aria-label="建立主故事版本缺少的条件">
      <p className="creation-source-message"><strong>建立主故事版本还需要：</strong></p>
      <ul>
        <li>{storyUnits.length ? "✓ 至少一个故事单元（已就绪）" : "✗ 至少一个故事单元——到事件线“常规创作”新建，创建后即为可用单元"}</li>
        <li>{state.availableEvents.length ? `✓ 所选单元已有 ${state.availableEvents.length} 个已确认事件` : "✗ 所选故事单元至少有一个已确认事件——在事件线“常规创作”里完成候选、影响评审并作者确认"}</li>
      </ul>
      <div className="creation-source-missing-actions">
        <button type="button" onClick={() => { window.location.assign("/event-line"); }}>前往事件线准备</button>
        <button type="button" onClick={() => { window.location.assign("/library"); }}>前往资料库</button>
      </div>
    </section> : null}
    {!state.root ? <button type="button" className="primary-action" disabled={busy || Boolean(sourceRequestBlocker) || !state.availableEvents.length || !storyUnits.length} onClick={() => act("create-root")}><GitBranch />建立主故事版本</button> : state.packageMode === "current-selection" ? <button type="button" className="primary-action" disabled={busy || !state.package} onClick={() => act("create-artifact")}><FilePlus2 />建立新的固定创作稿</button> : null}
    {state.package ? <section className="creation-source-package" aria-label="中性故事包"><header><div><small>{state.packageMode === "pinned-artifact" ? "固定创作稿 · 作者阅读版预览" : "当前选择 · 作者阅读版预览"}</small><h2>{state.package.scope.label}</h2><p><code>{state.package.id}</code> · <code>{state.package.digest}</code></p></div><button type="button" className="primary-action" onClick={download} disabled={!currentScopeMatchesState}> <Download />下载 Markdown</button></header>{!currentScopeMatchesState ? <p className="creation-source-message" role="status">正在按新的选择更新来源包；旧范围不能下载。</p> : null}{state.package.warnings.length ? <p className="creation-source-message">{state.package.warnings.join("；")}</p> : null}<p>{state.packageMode === "pinned-artifact" ? "下载内容严格来自这份创作稿建立时保存的快照。" : "这是当前选择的新版本预览；它不会改写既有固定稿。"} 预览与下载共用同一份冻结 Markdown；来源说明位于正文之后，下载文件另附包标识、内容摘要与来源回执索引。</p><pre>{state.package.storyMarkdown}</pre></section> : <p className="creation-source-message">{state.root ? "当前主版本还缺少可验证的故事单元或已确认事件，暂不能导出。" : "建立主故事版本后将显示中性故事包预览。"}</p>}
    </> : null}
  </section></main>;
}
