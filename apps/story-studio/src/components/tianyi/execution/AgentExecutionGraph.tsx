...
function AgentResultNode(props: NodeProps<Node<ExecutionNodeData>>) {
  return <NodeShell family="agent-result" status={props.data.status} selected={props.selected} ariaLabel={`${props.data.title}，${props.data.summary}`}><GraphPort type="target" position={Position.Left} connectable={false} label="结果输入" /><GraphPort type="source" position={Position.Right} connectable={false} label="进入作者审核" /><span className="agent-result-mark"><Braces aria-hidden="true" /></span><div><small>推演结果 · 仍是候选</small><strong>{props.data.title}</strong><span>{props.data.summary}</span><button type="button" onClick={(event) => { event.stopPropagation(); props.data.onOpenCandidates?.(); }}>回到事件图审阅候选</button></div><NodeStatus status={props.data.status} /></NodeShell>;
}
