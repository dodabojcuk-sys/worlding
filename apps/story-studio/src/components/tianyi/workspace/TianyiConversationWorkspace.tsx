...
export function TianyiConversationWorkspace(props: { runtime: TianyanShellRuntimeState; onOpenPendingReview(): void }) {
  ...
  const refreshWorkContext = useCallback(async () => {
    ...
    try {
      ...
      setSelectedMaterialIds((current) => current.filter((id) => materials.some((item) => item.id === id)).slice(0, MAX_EXPLICIT_MATERIAL_REFS));
  ...
  const openGroundedMaterial = (sourceId: string) => {
    const current = new URL(window.location.href);
    window.location.assign(`/library?materialId=${encodeURIComponent(sourceId)}&materialReturn=${encodeURIComponent(`${current.pathname}${current.search}`)}`);
  };
        ...
        /> : <section className="tianyi-lane-stage" aria-label={t("tianyi.workspace.workMode")}>
          ...
          /> : activeIntakeResolution ? <div className="story-intake-recovery" role="alert"><strong>无法恢复原候选</strong><p>{storyIntakeRecoveryMessage(activeIntakeResolution.status)}</p><button type="button" onClick={() => changeLane("review")}>返回当前批次审阅</button></div> : <>
            ...
            {!activeLegacyCandidate ? <details className="tianyi-work-context-picker" open>
              ...
              <fieldset className="tianyi-material-context-picker"><legend>作者明确资料引用（至多 {MAX_EXPLICIT_MATERIAL_REFS} 项）</legend><p>预览不发送模型；点击发送时会重新核对项目、对象和修订。规则在这里作为作者选择的证据，不会自动升级为场景硬约束。</p>{workMaterials.length ? workMaterials.map((material) => <label key={material.id}><input type="checkbox" checked={selectedMaterialIds.includes(material.id)} onChange={() => toggleMaterial(material.id)} />{material.title} · {material.type} <small>修订 {material.revisionToken.slice(0, 12)}</small><span>{material.body.slice(0, 120) || "（正文为空）"}</span></label>) : <p>当前作品没有可引用的资料，或资料尚在读取。</p>}</fieldset>
