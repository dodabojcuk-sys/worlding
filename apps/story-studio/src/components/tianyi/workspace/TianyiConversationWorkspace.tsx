      ...
      <section className="tianyi-conversation-column">
        ...
        /> : <section className="tianyi-lane-stage" aria-label={t("tianyi.workspace.workMode")}>
          ...
          /> : activeIntakeResolution ? <div className="story-intake-recovery" role="alert"><strong>无法恢复原候选</strong><p>{storyIntakeRecoveryMessage(activeIntakeResolution.status)}</p><button type="button" onClick={() => changeLane("review")}>返回当前批次审阅</button></div> : <>
            <div className="tianyi-stage-heading"><div><small>WORK LANE</small><h2>{activeLegacyCandidate?.title ?? "当前故事工作上下文"}</h2></div><div>{materialReturn ? <button type="button" onClick={() => window.location.assign(materialReturn)}>返回资料</button> : null}<span>{activeLegacyCandidate ? t("tianyi.workspace.workGuide") : "先明确范围，再决定是否需要整理候选"}</span></div></div>
            ...
            {activeLegacyCandidate ? <TianyiAdoptionPanel runtime={runtime} onOpenEventLine={openEventLine} /> : <>
              ...
              {lastGroundedAnswer ? <section className="tianyi-grounded-answer-receipt" aria-label="本问来源回执"><header><div><small>已保存回答回执</small><h3>“{lastGroundedQuestion}”</h3></div><span>{lastGroundedAnswer.providerDispatchCount} 次模型发送</span></header><p>{lastGroundedAnswer.answer?.summary}</p><dl><div><dt>Receipt</dt><dd>{lastGroundedAnswer.receiptId}</dd></div><div><dt>来源清单校验</dt><dd>{lastGroundedAnswer.sourceManifest.digest}</dd></div></dl><ul>{lastGroundedAnswer.includedSources.map((source) => { const material = workMaterials.find((item) => item.id === source.sourceId); const label = material?.title ?? source.sourceId; return <li key={source.sourceKey}>{lastGroundedAnswer.sourceManifest.request.eventRefs?.includes(source.sourceKey) ? <button type="button" onClick={() => openGroundedEvidenceEvent(source)}>{label}</button> : source.sourceType === "map" ? <button type="button" onClick={openGroundedMap}>{label}</button> : source.sourceType === "world-object" || source.sourceType === "rule" ? <button type="button" onClick={() => openGroundedMaterial(source.sourceId, source.contentHash)}>{label}</button> : <strong>{label}</strong>}<span>采用修订 {source.contentHash.slice(0, 12)} · {source.lane}</span>{material?.revisionToken === source.contentHash ? <details><summary>实际采用正文</summary><p>{material.body}</p></details> : <small>资料后来已有修改；打开来源可读取这次回答采用的历史正文。</small>}</li>; })}</ul></section> : null}
            </>}
          </>}
        </section>}
