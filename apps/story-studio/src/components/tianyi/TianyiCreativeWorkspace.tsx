...
export function TianyiCreativeWorkspace(props: { projectId: string; token: string; sessionId: string | null; operations: TianyiV2Operations; onSessionId(sessionId: string): void; sharedDraft: string; onDraft(value: string): void; onOpenEventLine?(eventId: string): void; withConnection?: <T>(action: (token: string) => Promise<T>) => Promise<T> }) {
  ...
  const handoff = async (candidateId: string) => { if (!props.sessionId) return; setBusy(true); setError(""); try { const result = await withToken((token) => props.operations.handoffCreative({ projectId: props.projectId, sessionId: props.sessionId!, candidateId, operationId: id("handoff"), token })); setProjection(result.projection); } catch (cause) { setError(cause instanceof Error ? cause.message : "现有资料交接未完成。"); } finally { setBusy(false); } };
  const readEventReview = async (candidateId: string) => { if (!props.sessionId) return; setBusy(true); setError(""); try { const value = await withToken((token) => props.operations.readCreativeEventReview({ projectId: props.projectId, sessionId: props.sessionId!, candidateId, token })); setEventReview(value); setSelectedImpactOptionId(value.impact?.options[0]?.id || ""); } catch (cause) { setError(cause instanceof Error ? cause.message : "事件候选审查不可用。"); } finally { setBusy(false); } };
  ...
  const confirmEvent = async (candidateId: string) => { if (!props.sessionId || !selectedImpactOptionId) return; setBusy(true); try { const value = await withToken((token) => props.operations.confirmCreativeEvent({ projectId: props.projectId, sessionId: props.sessionId!, candidateId, optionId: selectedImpactOptionId, token })); setEventReview(value); const eventId = value.confirmedEvents[0]?.id; if (eventId) props.onOpenEventLine?.(eventId); } catch (cause) { setError(cause instanceof Error ? cause.message : "正式写入未完成。"); } finally { setBusy(false); } };
  ...
  useEffect(() => { if (selectedCandidate?.ownerReceipt?.owner === "candidate-review") void readEventReview(selectedCandidate.candidateId); else setEventReview(null); }, [selectedCandidate?.candidateId, selectedCandidate?.ownerReceipt?.owner]);
  ...
  return <div className={`tianyi-creative-canvas ${reviewOpen ? "is-review-open" : ""}`} data-testid="tianyi-creative-workspace"><section className="tianyi-creative-timeline" aria-label="作者原话与天意创意协作">
    ...
    </section><aside className={`tianyi-creative-review ${reviewOpen ? "is-open" : ""}`} aria-label="创意候选审查"><header><span>待审候选</span><strong>{projection?.pendingCount ?? 0}</strong><button type="button" className="tianyi-mobile-review-close" onClick={() => { setReviewOpen(false); reviewTriggerRef.current?.focus(); }}>返回画布</button></header>{projection?.candidates.length ? <><nav className="tianyi-creative-candidate-nav" aria-label="候选列表">{projection.candidates.map((candidate) => <button key={candidate.candidateId} type="button" className={candidate.candidateId === selectedCandidate?.candidateId ? "is-selected" : ""} onClick={() => { setSelectedCandidateId(candidate.candidateId); setEditingCandidateId(null); }}><span>{candidate.title}</span><small>{candidate.state === "pending" ? "待审" : candidate.state}</small></button>)}</nav>{selectedCandidate ? <article key={selectedCandidate.candidateId}><small>{selectedCandidate.kind} · {selectedCandidate.state === "pending" ? "待作者审查" : selectedCandidate.state}</small>{editingCandidateId === selectedCandidate.candidateId ? <div className="tianyi-creative-edit"><label>候选名称<input value={editTitle} onChange={(event) => setEditTitle(event.target.value)} /></label><label>候选说明<textarea value={editSummary} onChange={(event) => setEditSummary(event.target.value)} /></label><footer><button type="button" className="primary-action" disabled={busy} onClick={() => void saveEdit(selectedCandidate)}>保存编辑</button><button type="button" className="secondary-action" disabled={busy} onClick={() => setEditingCandidateId(null)}>取消</button></footer></div> : <><strong>{selectedCandidate.title}</strong><p>{selectedCandidate.summary.replace(/现有 Agent\/Object/gu, "现有资料")}</p></>}<em>来源：{selectedCandidate.sourceExcerpt}</em><span>后续归属：{selectedCandidate.targetOwnerKind === "agent-recognition-proposal" ? "现有资料审核" : selectedCandidate.targetOwnerKind === "candidate-review" ? "事件候选审查" : "继续保留为候选"}</span>{selectedCandidate.ownerReceipt ? <span>已有资料回执：{selectedCandidate.ownerReceipt.id} · r{selectedCandidate.ownerReceipt.revision ?? "?"}</span> : null}{selectedCandidate.uncertainties.map((item) => <span key={item}>待确认：{item}</span>)}{eventReview && selectedCandidate.targetOwnerKind === "candidate-review" ? <section className="tianyi-creative-event-review"><strong>事件候选审查</strong><p>来源版本：{eventReview.proposal.origin.version}</p><p>写入目标：{eventReview.proposal.writeTarget.storyId}</p><p>候选状态：{eventReview.candidateReview?.status || "待建立"}</p>{eventReview.impact ? <><p>影响审查：{eventReview.impact.status}</p><select aria-label="影响审查路线" value={selectedImpactOptionId} onChange={(event) => setSelectedImpactOptionId(event.target.value)}>{eventReview.impact.options.map((option) => <option key={option.id} value={option.id}>{option.label}：{option.summary}</option>)}</select></> : null}{eventReview.confirmedEvents[0] ? <button type="button" className="primary-action" onClick={() => props.onOpenEventLine?.(eventReview.confirmedEvents[0].id)}>在事件线查看正式事件</button> : <footer>{!eventReview.impact ? <button type="button" className="primary-action" disabled={busy} onClick={() => void beginEventImpact(selectedCandidate.candidateId)}>查看影响审查</button> : <button type="button" className="primary-action" disabled={busy || !selectedImpactOptionId} onClick={() => void confirmEvent(selectedCandidate.candidateId)}>作者确认并写入事件线</button>}<button type="button" className="secondary-action" disabled={busy} onClick={() => void rejectEvent(selectedCandidate.candidateId)}>拒绝候选</button></footer>}</section> : null}{selectedCandidate.state === "pending" && <footer>{selectedCandidate.targetOwnerKind === "agent-recognition-proposal" ? <button type="button" className="primary-action" disabled={busy} onClick={() => void handoff(selectedCandidate.candidateId)}>送入现有资料审核</button> : selectedCandidate.targetOwnerKind === "candidate-review" ? <button type="button" className="primary-action" disabled={busy} onClick={() => void handoff(selectedCandidate.candidateId)}>送入事件候选审查</button> : null}<button type="button" className="secondary-action" disabled={busy} onClick={() => beginEdit(selectedCandidate)}>编辑</button><button type="button" className="secondary-action" disabled={busy} onClick={() => void decide(selectedCandidate.candidateId, "deferred")}>稍后处理</button><button type="button" className="secondary-action" disabled={busy} onClick={() => void decide(selectedCandidate.candidateId, "rejected")}>拒绝</button></footer>}</article> : null}</> : <p>整理后，候选会显示在这里；没有唯一归属的内容继续保留为候选。</p>}<footer className="tianyi-creative-session-actions"><button type="button" className="secondary-action" onClick={() => void pause()} disabled={!props.sessionId || busy || projection?.lifecycle === "completed" || projection?.lifecycle === "archived"}><CirclePause />暂停</button>{projection?.lifecycle === "paused" || projection?.lifecycle === "provider-unavailable" || projection?.lifecycle === "recovering" ? <button type="button" className="secondary-action" onClick={() => void recover()} disabled={busy}>恢复安全点</button> : null}<button type="button" className="primary-action" onClick={() => void complete()} disabled={!props.sessionId || busy || (projection?.pendingCount ?? 0) > 0 || projection?.lifecycle === "completed" || projection?.lifecycle === "archived"}><Check />整理本次创意</button></footer></aside></div>;
}
