...
import { inspectMapNorthRelation } from "../../../../../../src/storyContracts/mapCalibration.ts";
...
export function MapAiCollaborationPanel(props: { runtime: TianyanShellRuntimeState; context: TianyiMapEditContext; ensureConversation(): Promise<string> }) {
  ...
  const [preserveLineEndpoints, setPreserveLineEndpoints] = useState(false);
  ...
  useEffect(() => {
    ...
    setMap(null); setProposal(null); setSummary(""); setError(""); setOutcome(null); setReferences([]); setAvoidAreaReferences([]); setPreserveLineEndpoints(false);
    if (!projectId) return;
    let active = true;
  ...
  const generate = () => void (async () => {
    ...
    try {
      ...
      const result = await props.runtime.withConnection((token) => generateMapEditProposal({
        ...
        referenceObjectIds: references, avoidAreaObjectIds: avoidAreaReferences, preserveLineEndpoints, token, signal: abort.signal
      }));
  ...
  const directionInspection = useMemo(() => {
    const request = props.runtime.workComposerDraft.trim();
    if (!/(?:检查|是否|一致|矛盾|冲突)[\s\S]{0,80}(?:北侧|北边|北面|以北)|(?:北侧|北边|北面|以北)[\s\S]{0,80}(?:检查|是否|一致|矛盾|冲突)/u.test(request)) return null;
    if (!map?.content.coordinateSystem.north) return { state: "unknown", message: "这张地图尚未设置北向；只能描述画布上下，不能判断南北。" };
    if (mode !== "selection" || editable.length !== 1 || references.length !== 1) return { state: "unknown", message: "请选择恰好一个待检查对象和一个只读参照对象；重名或多选时不会猜测。" };
    const reference = map.content.drawings.find((item) => item.id === references[0]);
    if (!reference) return { state: "unknown", message: "所选参照对象已失效，请重新选择。" };
    const result = inspectMapNorthRelation({ targetCanvasPoints: editable[0].points, referenceCanvasPoints: reference.points, bounds: map.content.coordinateSystem.bounds, north: map.content.coordinateSystem.north });
    const targetLabel = editable[0].label ?? editable[0].subtype;
    const referenceLabel = reference.label ?? reference.subtype;
    return result.status === "north"
      ? { state: "aligned", message: `${targetLabel}确实位于${referenceLabel}的地图北侧；依据是作者设置的北向和两项当前几何。` }
      : result.status === "south"
        ? { state: "conflict", message: `文字要求${targetLabel}位于${referenceLabel}北侧，但当前地图几何显示它在南侧。` }
        : { state: "unknown", message: `${targetLabel}与${referenceLabel}在南北方向上的差异太小，无法可靠判断。` };
  }, [editable, map, mode, props.runtime.workComposerDraft, references]);
...
function RequestEditor(props: any) {
  ...
  return <>
    ...
    {props.mode === "selection" ? <><section className="tianyi-map-scope-card"><b>可修改</b>{props.editable.length ? props.editable.map((item: any) => <span key={item.id}>{item.label ?? item.subtype}</span>) : <p>地图页面没有选择可编辑图形；不会扩大为整张地图。</p>}</section><label><input type="checkbox" checked={props.preserveLineEndpoints} onChange={(event) => props.setPreserveLineEndpoints(event.target.checked)}/>固定所选线条的两端</label><small>可选约束。未勾选时允许调整整条线；已勾选时首尾点改变会拒绝整个提案。</small></> : <section className="tianyi-map-region-controls"><label>可编辑图层<select value={props.layerId} onChange={(event) => props.setLayerId(event.target.value)}>{writableLayers.map((layer: any) => <option key={layer.id} value={layer.id}>{layer.title}</option>)}</select></label><fieldset><legend>明确区域（地图坐标）</legend>{(["x", "y", "width", "height"] as const).map((key) => <label key={key}>{key === "x" ? "左" : key === "y" ? "上" : key === "width" ? "宽" : "高"}<input type="number" min="0" max="100" value={props.bounds[key]} onChange={(event) => props.setBounds((value: any) => ({ ...value, [key]: Number(event.target.value) }))}/></label>)}</fieldset><small>虚线框是本次唯一可新增范围；既有对象全部只读。</small></section>}
...
function ReviewBody(props: any) {
  ...
  return <section className={`tianyi-map-review is-${proposal.status}`}>
    ...
    <section className="tianyi-map-review-checks"><h3>系统已经检查</h3><ul>{proposal.spatialChecks?.map((check) => <li key={check.kind}><Check aria-hidden="true"/>{check.kind === "line-endpoints-preserved" ? "道路起点和终点与修改前完全一致" : check.kind === "north-of-reference" ? "建议对象位于所选参照对象的地图北侧" : "建议道路未接触或进入作者明确选择的范围多边形"}</li>)}<li><Check aria-hidden="true"/>修改对象、图层、坐标和操作数量均在授权范围内</li></ul><h3>仍需作者判断</h3><p>位置是否符合地标、叙事意图和视觉质量；模型理由只是建议，不是系统检查结果。</p></section>
