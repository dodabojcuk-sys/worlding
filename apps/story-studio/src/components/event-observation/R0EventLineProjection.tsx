...
export function R0EventLineProjection(props: { runtime: TianyanShellRuntimeState; onOpenTianyi(reference?: StoryStudioEventReference | StoryStudioEventReference[], initialDraft?: string, predictionSourceLabels?: string[], predictionSourceUnitSummary?: string, knowledgeView?: TianyiKnowledgeViewContext): void; selectedEventId?: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    firstFeedbackAt.current = navigationStartedAt.current;
    setInteractiveAt(null);
    ...
    try {
      ...
      const [library, list, units, relationList, relationTypes, creationSource] = await Promise.all([getWorldLibrary(project.id), getVerifiedCanonEventList(project.id, workVersionId), listStoryUnits(project.id), listRelations({ projectId: project.id, ...(workVersionId ? { workVersionId } : {}) }), listRelationTypes(project.id), getCreationSourcePortState({ projectId: project.id })]);
  ...
  const tianyiCandidateActive = Boolean(props.runtime.tianyiConversationId && props.runtime.activeTianyiCandidateId);
  const returnToTianyi = () => {
    const params = new URLSearchParams({ tianyiLane: "work" });
    if (props.runtime.tianyiConversationId) params.set("tianyiSession", props.runtime.tianyiConversationId);
    if (props.runtime.activeTianyiCandidateId) params.set("tianyiCandidate", props.runtime.activeTianyiCandidateId);
