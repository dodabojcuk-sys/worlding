...
export function R0EventLineProjection(props: { runtime: TianyanShellRuntimeState; onOpenTianyi(reference?: StoryStudioEventReference | StoryStudioEventReference[], initialDraft?: string, predictionSourceLabels?: string[], predictionSourceUnitSummary?: string, knowledgeView?: TianyiKnowledgeViewContext): void; selectedEventId?: string | null }) {
  ...
  }).then(async () => { window.dispatchEvent(new Event("story-studio-pending-review-changed")); await load(); })} onRejectGraphRelation={(relation) => props.runtime.withConnection((token) => rejectRelationCandidate({ projectId: state.projectId!, relationId: relation.relationId, expectedRelationRevision: relation.revision, operationId: `event-graph-reject-${relation.relationId}-${relation.revision}`, token })).then(() => { window.dispatchEvent(new Event("story-studio-pending-review-changed")); load(); })} onContinueReview={() => undefined} /></div>;
}
