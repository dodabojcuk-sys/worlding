...
export function EventObservationControls(props: {
  ...
  return <section className="event-observation-controls" aria-label="事件观察组合">
    <div className="event-observation-axis">
      <span>布局坐标</span>
      <div role="group" aria-label="布局坐标">
    ...
    <div className="event-observation-axis">
      <span>主观察镜头</span>
      <div role="group" aria-label="主观察镜头">
    ...
    <div className="event-observation-options">
      <label><span>语义尺度</span><select value={props.state.scale} onChange={(event) => props.onScale(event.target.value as EventObservationScale)}><option value="story">故事</option><option value="unit">单元</option><option value="event">事件</option></select></label>
