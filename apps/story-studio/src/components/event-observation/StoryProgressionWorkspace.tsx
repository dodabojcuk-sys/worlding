...
export function StoryProgressionWorkspace(props: {
  ...
  return <section className="story-progression-workspace" data-testid="story-progression-workspace" data-event-task={props.task} data-arrangement-state={hasArrangement ? placed.length ? "placed" : "formal-empty" : "unplaced"}>
    <header className="story-progression-heading">
      <h1>{title}</h1><span>{props.currentUnitLabel ?? "全部故事单元"} · {props.events.length} 个事件 · {placed.length} 个正式位置</span>
      <details className="story-progression-help"><summary>说明</summary><p>{summary}{conflicts.length ? ` 当前还有 ${conflicts.length} 个编排冲突。` : ""}</p></details>
    </header>
    ...
    <nav className="story-progression-controls" aria-label="事件线任务">
      ...
      <div className="story-progression-actions">
        <details className="story-progression-filters"><summary>范围与观察</summary><div><label className="story-progression-coordinate"><span>故事线范围</span><select data-testid="storyline-scope-select" value={props.storylineScope} onChange={(event) => props.onStorylineScope(event.target.value)}><option value="all">全部故事线</option>{props.storylineScope !== "all" && !props.storylines.some((line) => line.id === props.storylineScope) ? <option value={props.storylineScope}>正在恢复故事线…</option> : null}{props.storylines.map((line) => <option key={line.id} value={line.id}>{line.label} · {line.eventIds.length}</option>)}</select></label><label className="story-progression-coordinate"><span>观察者 / 知情视角</span><select data-testid="knowledge-observer-select" value={props.comparisonMode ? "author" : props.observerId} onChange={(event) => props.onObserver(event.target.value)}>{props.comparisonMode ? <option value="author">比较视角（{props.observerIds.length} 人）</option> : null}{props.observers.map((observer) => <option key={observer.id} value={observer.id}>{observer.label}</option>)}</select></label><CharacterObservationTray projectId={props.projectId} workVersionId={activeWorkVersionId} characters={formalCharacters} selectedIds={selectedCharacterIds} message={observationMessage} onMessage={setObservationMessage} onChange={(ids) => { setObservationMessage(null); props.onObservers(ids); }} onOpenPicker={() => setCompareOpen(true)} /></div></details>
