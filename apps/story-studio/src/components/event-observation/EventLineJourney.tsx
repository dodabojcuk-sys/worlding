...
import { eventLineJourneyLines, eventLineJourneyNodes, eventLineJourneyUrl, readEventLineJourneyLocation, type EventLineJourneyLocation } from "./eventLineJourneyModel";
...
export function EventLineJourney(props: Props) {
  ...
  const linkedUnitIds = new Set(lines.flatMap((item) => item.units.map((unit) => unit.id)));
  const unlinkedUnits = props.units.filter((unit) => unit.lifecycle !== "archived" && !linkedUnitIds.has(unit.id));
  ...
  const unitNodes = unit ? eventLineJourneyNodes(unit, props.events, props.narratives) : [];
  const positions = new Map(props.narratives.flatMap((item) => item.projection.placed).filter((item) => item.storyUnitId === unit?.id).map((item) => [item.eventId, item.narrativeIndex]));
