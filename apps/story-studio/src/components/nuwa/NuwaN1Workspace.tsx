...
function ContextInspector(props: { actors: Array<{ actorId: string; actorLabel: string; localGoal: string; coreSummary: string; profileBasis: { core: string | null; boundaries: string | null; sourceRevision: string }; attention: { selected: Array<{ sourceId: string; reason: string }>; excluded: { count: number }; budget: { estimator: string; maxInputTokens: number; baseBytes: number; sourceBudgetBytes: number; selectedSourceBytes: number; outputReserveTokens: number; requiredOverflow: boolean } }; knowledgeItems: Array<{ id: string; summary: string; visibility: string }>; beliefItems: Array<{ id: string; summary: string; stance: string }>; memoryItems: Array<{ id: string; summary: string; source: { speakerId: string; sourceRunId: string; sourceStepId: string; sceneId: string; sceneObservedAt: string; workVersionId: string; workRevision: string; validity: "active" }; selectedByAttention?: boolean }>; evidenceRefs: string[]; excludedCount: number }> }) {
  ...
  return <section className="nuwa-n1-context-list">{props.actors.map((actor) => <article key={actor.actorId}><header><strong>{actor.actorLabel}</strong><small>实际发送上下文</small></header><dl className="nuwa-n1-context-basis"><div><dt>本场目标</dt><dd>{actor.localGoal}</dd></div><div><dt>角色核心</dt><dd>{actor.profileBasis.core ?? "未设置"}</dd></div><div><dt>底线</dt><dd>{actor.profileBasis.boundaries ?? "未设置"}</dd></div><div><dt>人物修订</dt><dd>{actor.profileBasis.sourceRevision}</dd></div></dl>{actor.attention.selected.length ? <ol className="nuwa-n1-attention-sources">{actor.attention.selected.map((source) => <li key={`${source.sourceId}:${source.reason}`}><span>{source.sourceId}</span><small>{attentionReasonLabel(source.reason)}</small></li>)}</ol> : <p>预算内没有额外可发送依据。</p>}<p>{actor.knowledgeItems.length ? actor.knowledgeItems.map((item) => item.summary).join("；") : "当前没有可安全提供的已知内容。"}</p>{actor.beliefItems.length ? <p><small>信念与误解</small><br />{actor.beliefItems.map((item) => item.summary).join("；")}</p> : null}{actor.memoryItems.length ? <div className="nuwa-n1-memory-sources"><small>跨场景听闻记忆（仍是 heard，不是世界事实）</small><ul>{actor.memoryItems.map((item) => <li key={item.id}><span>{item.summary}</span><small>{item.source.sceneId} · {item.source.sceneObservedAt} · {item.source.workVersionId}@r{item.source.workRevision} · {item.source.validity === "active" ? "当前有效" : item.source.validity}{item.selectedByAttention === false ? " · 本回合未进入注意力" : ""}</small></li>)}</ul></div> : null}<div className="nuwa-n1-attention-budget"><small>保守预算</small><span>{actor.attention.budget.selectedSourceBytes} / {actor.attention.budget.sourceBudgetBytes} 字节来源额度 · 完整输入上限 {actor.attention.budget.maxInputTokens} · 输出预留 {actor.attention.budget.outputReserveTokens}</span></div><footer><span>{actor.evidenceRefs.length} 条已选来源</span>{actor.attention.excluded.count ? <span>{actor.attention.excluded.count} 条低相关来源未发送</span> : null}{actor.excludedCount ? <span>{actor.excludedCount} 项权限排除（身份隐藏）</span> : null}</footer></article>)}</section>;
}
