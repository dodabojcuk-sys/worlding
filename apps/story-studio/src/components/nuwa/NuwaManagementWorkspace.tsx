...
export function NuwaManagementWorkspace(props: { runtime: TianyanShellRuntimeState }) {
  ...
  return <main className="shell-workspace shell-workspace-nuwa nuwa-management" aria-label="女娲管理">
    ...
    {error ? <p role="alert">排演列表暂时无法读取；原 Run 未改变。</p> : !bootstrap ? <p role="status">正在读取排演记录…</p> : <>
      <section aria-label="当前与历史排演"><h2><History aria-hidden="true" />当前与历史排演</h2>{runs.length ? <ol>{runs.map((run) => <li key={run.runId}><a href={`/nuwa?runId=${encodeURIComponent(run.runId)}`} title={run.runId}><strong>{run.label}{run.runId === current?.runId ? " · 当前" : ""}</strong><span>{runStatusLabel[run.status] ?? run.status} · {new Date(run.createdAt).toLocaleString()}</span></a></li>)}</ol> : <p>还没有排演；可以从新建流程选择故事范围。</p>}</section>
    </>}
  </main>;
}
