...
export function EventLineWorkbench(props: {
  ...
  const currentEvent = props.events.find((event) => metadataById[event.id]?.unitLabel === props.currentUnitLabel) ?? props.events[0] ?? null;
  ...
  const revealCurrentEvent = () => {
    ...
    setFilter({ kind: "all" });
    window.requestAnimationFrame(() => document.querySelector<HTMLElement>(`[data-confirmed-event-id="${CSS.escape(currentEvent.id)}"]`)?.scrollIntoView({ block: "center" }));
  };
        ...
        {props.listState.status === "ready" && visibleEvents.length > 0 ? <div className={`event-line-spine ${compact ? "is-compact" : ""}`} data-testid="confirmed-story-spine" aria-label="已确认故事脊柱">
          {groupedEvents.map((group) => <section className="event-line-unit" key={group.label} data-current-unit={group.label === props.currentUnitLabel ? "true" : "false"}>
            ...
            {group.setPoints.map((setPoint) => <section className="event-line-set-point" key={setPoint.label}><header><span><CircleDot /></span><div><small>集点</small><h3>{setPoint.label}</h3></div><strong>{setPoint.events.length} 个节点</strong></header><ol>{setPoint.events.map((event) => <EventSpineNode
                ...
                current={event.id === currentEvent?.id}
...
function EventLineListState(props: { state: VerifiedCanonEventListRead | { status: "loading" }; invalidRecordCount: number; eventCount: number; onRetry(): void }) {
  ...
  if (props.invalidRecordCount > 0) return <><div className="event-line-state is-warning" role="alert" data-testid="event-line-invalid-records"><strong>发现 {props.invalidRecordCount} 条未通过验证的确认外观记录</strong><span>这些记录已排除，不会伪装成正式事实。</span><button type="button" onClick={props.onRetry}><RefreshCw />重新核验</button></div>{props.eventCount === 0 ? <p className="event-line-state" data-testid="event-line-no-verified-records">当前没有通过完整作者确认链验证的事件。</p> : null}</>;
