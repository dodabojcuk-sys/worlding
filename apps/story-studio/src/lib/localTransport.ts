...
export async function handoffTianyiCreativeCandidate(input: { projectId: string; sessionId: string; candidateId: string; operationId: string; token: string }): Promise<{ projection: TianyiCreativeProjection; ownerReceipt: { owner: string; id: string; revision: number } }> { const { token, ...body } = input; return tianyiRequest("creative/candidate/handoff", token, body); }
