...
export type DocumentRevision = { id: string; sequence: number; contentHash: string; source: "create" | "save" | "restore" | "external-baseline"; recordedAt: string; restoredFromRevisionId: string | null; operationId?: string | null };
...
export type MapPlacement = { id: string; childMapId: string; kind: "point" | "range" | "calibrated"; point: { x: number; y: number } | null; bounds: Array<{ x: number; y: number }>; transform: null | { translateX: number; translateY: number; rotation: number; scale: number }; calibration?: null | { sourcePoints: [{ x: number; y: number }, { x: number; y: number }]; targetPoints: [{ x: number; y: number }, { x: number; y: number }] }; precision: "illustrative" | "calibrated"; note: string | null };
...
export type NuwaBranchReadModel = { version: string; branch: NuwaBranchSummary; scenes: NuwaBranchScene[]; nodes: NuwaBranchNode[] };
...
export async function createNuwaBranch(input: { projectId: string; displayName: string; runId?: string; handoffId?: string | null; operationId: string; token: string }): Promise<{ version: string; branch: NuwaBranchSummary }> {
  const { token, ...body } = input;
...
export async function checkpointNuwaBranch(input: { projectId: string; branchWorkVersionId: string; idempotencyKey: string; operationId: string; token: string }): Promise<{ version: string; branch: NuwaBranchSummary; checkpoint: { workVersionReceiptId: string; revision: number; provenanceCount: number } }> {
  const { token, ...body } = input;
  return request<{ version: string; branch: NuwaBranchSummary; checkpoint: { workVersionReceiptId: string; revision: number; provenanceCount: number } }>(`${basePath}/nuwa-branch/checkpoint`, { method: "POST", token, body });
}
...
export async function listMultiNodePredictionReviews(projectId: string, runId?: string): Promise<MultiNodePredictionReviewProjection[]> { return request(`${basePath}/author-control/prediction-review?projectId=${encodeURIComponent(projectId)}${runId ? `&runId=${encodeURIComponent(runId)}` : ""}`); }
