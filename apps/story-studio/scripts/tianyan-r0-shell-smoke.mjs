...
async function assertNuwaN1BoundedLoop(page, consoleProblems) {
  const evidenceDirectory = nuwaN1EvidenceDirectory || r5ContinuousEvidenceDirectory;
  if (evidenceDirectory) mkdirSync(evidenceDirectory, { recursive: true });
  const evidenceDwell = async () => {
    if (evidenceDirectory) await page.waitForTimeout(nuwaN1EvidenceDwellMs);
  };
  ...
  const objectsBefore = await getFixture(`${apiUrl}/__local/story-studio/world-library?projectId=${encodeURIComponent(fixtureProjectId)}`);
  ...
  assert.match(await workspace.locator(".nuwa-n1-participant-options").innerText(), /林昭/u);
  ...
  const stepActors = await steps.locator(".nuwa-n1-step-content > header > button").allTextContents();
  ...
  await workspace.getByRole("button", { name: "故事结构", exact: true }).click();
  await page.waitForURL(/\/event-line/u);
  await page.getByTestId("event-line-journey").waitFor();
  assert.equal(await page.getByTestId("event-line-journey").getAttribute("data-level"), "lines", "Story structure starts with line selection in the central workspace.");
  await page.goBack();
  await page.getByTestId("nuwa-n1-workspace").waitFor();
  assert.equal(await page.locator(".nuwa-n1-reader li").count(), 2, "Returning from story browsing keeps the saved Run steps.");
  const nuwaDirectory = page.getByTestId("nuwa-unified-directory");
  await nuwaDirectory.getByRole("button", { name: "收起目录" }).click();
  if (await steps.locator('.nuwa-n1-step-select input').count()) await workspace.getByRole("button", { name: /完成选择/u }).click();
  assert.equal(await steps.locator('.nuwa-n1-step-select input').count(), 0, "Result checkboxes stay out of the reading flow.");
  await workspace.getByRole("button", { name: /^选择结果/u }).click();
  if (!(await steps.first().locator('.nuwa-n1-step-select input').isChecked())) await steps.first().locator('.nuwa-n1-step-select input').check();
  await workspace.getByRole("button", { name: /完成选择.*已选/u }).click();
  ...
  assert.equal(await contextCards.count(), 1, "After returning, the selected step keeps its scoped actor context instead of showing every role at once.");
  assert.match(await contextCards.first().innerText(), /本步骤使用的依据/u, "The returned inspector remains bound to a saved step's role context.");
  await workspace.getByRole("button", { name: "角色与来源" }).click();
  ...
  await workspace.locator(".nuwa-n1-composer textarea").fill("只给女娲的草稿");
  await workspace.getByLabel("作者提示接收对象").selectOption("actors");
  assert.equal(await workspace.locator(".nuwa-n1-composer textarea").inputValue(), "", "Changing recipient does not carry the director draft into a role instruction.");
  await workspace.locator(".nuwa-n1-composer textarea").fill("只给角色的草稿");
  await workspace.getByLabel("作者提示接收对象").selectOption("nuwa");
  assert.equal(await workspace.locator(".nuwa-n1-composer textarea").inputValue(), "只给女娲的草稿");
  await workspace.getByLabel("作者提示接收对象").selectOption("actors");
  assert.equal(await workspace.locator(".nuwa-n1-composer textarea").inputValue(), "只给角色的草稿");
  await workspace.getByLabel("作者提示接收对象").selectOption("nuwa");
  ...
  await directorCard.waitFor();
  const directorDetails = directorCard.locator(".nuwa-n1-director-disclosure");
  ...
  assert.match(await directorDetails.innerText(), /导演调整[\s\S]*暂缓主动揭露[\s\S]*作者原话：[\s\S]*建议理解：[\s\S]*适用范围：/u, "完整审阅保存的建议、原话和作用范围后才能采纳。");
  ...
  const eventToggle = workspace.getByRole("button", { name: "排演脉络", exact: true });
  ...
  const pausedRunId = await workspace.getAttribute("data-run-id");
  await page.setViewportSize({ width: 390, height: 844 });
  await page.locator('.tianyan-mobile').waitFor();
  const phoneWorkspace = page.getByTestId("mobile-nuwa-template");
  await phoneWorkspace.waitFor();
  const phoneLayout = await phoneWorkspace.evaluate((node) => {
    const composer = node.querySelector(".mobile-nuwa-composer")?.getBoundingClientRect();
    return { overflow: document.documentElement.scrollWidth > innerWidth, composerBottom: composer?.bottom ?? Infinity };
  });
  assert.equal(phoneLayout.overflow, false, `Phone reading must not overflow horizontally: ${JSON.stringify(phoneLayout)}`);
  assert.ok(phoneLayout.composerBottom <= 844, `Phone composer remains reachable: ${JSON.stringify(phoneLayout)}`);
  await phoneWorkspace.getByRole("button", { name: /查看这一单元的故事结构/u }).click();
  await page.waitForURL(/\/event-line/u);
  await page.getByTestId("event-line-journey").waitFor();
  await page.goBack();
  await phoneWorkspace.waitFor();
  assert.equal(await phoneWorkspace.getAttribute("data-run-id"), pausedRunId, "Phone story browsing returns to the same paused Run.");
  await phoneWorkspace.getByText("排演操作与历史", { exact: true }).click();
  await phoneWorkspace.locator(".mobile-nuwa-process[open]").filter({ hasText: "排演操作与历史" }).waitFor();
  await phoneWorkspace.getByText("排演操作与历史", { exact: true }).click();
  await phoneWorkspace.getByRole("button", { name: /选择结果.*已选/u }).click();
  assert.equal(await phoneWorkspace.locator('.mobile-nuwa-step input:checked').count(), 1, "Phone selection mode preserves the saved choice.");
  await phoneWorkspace.getByRole("button", { name: /完成选择/u }).click();
  assert.equal(await phoneWorkspace.locator(".mobile-nuwa-composer textarea").inputValue(), "暂停刷新草稿不发送", "Phone navigation keeps the unsent author draft.");
  ...
  const compactGeometry = await workspace.evaluate((node) => {
    ...
    const inspector = node.querySelector(".nuwa-n1-inspector")?.getBoundingClientRect();
    const rail = document.querySelector(".shell-tool-rail")?.getBoundingClientRect();
    return { overflow: document.documentElement.scrollWidth > window.innerWidth, composerRight: composer?.right ?? 0, inspectorRight: inspector?.right ?? 0, railLeft: rail?.left ?? window.innerWidth };
  });
  ...
  if (evidenceDirectory) await workspace.locator(".nuwa-n1-reader").screenshot({ path: path.join(evidenceDirectory, "03a-five-message-reader.png") });
  await workspace.getByLabel("作者提示接收对象").selectOption("nuwa");
  await workspace.locator(".nuwa-n1-composer textarea").fill("删除已发生剧情并改写角色记忆");
  await workspace.getByRole("button", { name: "生成调整建议", exact: true }).click();
  if (await directorDetails.getAttribute("open") === null) await directorDetails.locator("summary").click();
  ...
  assert.equal(reviews.data.length, 1, "The selected N1 result reaches the existing pending-review owner.");
  assert.equal(reviews.data[0]?.status, "awaiting");
  const objectsAfter = await getFixture(`${apiUrl}/__local/story-studio/world-library?projectId=${encodeURIComponent(fixtureProjectId)}`);
  assert.equal(objectsAfter.data.objects.length, objectsBefore.data.objects.length, "N1 candidate handoff must not create a formal World object.");
...
async function assertRelationshipReaderR1(page, consoleProblems) {
  const evidenceDirectory = relationReaderEvidenceDirectory || r5ContinuousEvidenceDirectory;
  if (evidenceDirectory) mkdirSync(evidenceDirectory, { recursive: true });
  await page.setViewportSize({ width: 1440, height: 900 });
...
async function assertR5ContinuousAuthorLoop(page, consoleProblems) {
  if (r5ContinuousEvidenceDirectory) mkdirSync(r5ContinuousEvidenceDirectory, { recursive: true });
  // M2 and M4 start from one stable character reference, then the existing N1
  // fixture proves that the same project keeps two formal roles bounded.
  await assertRelationshipReaderR1(page, consoleProblems);
  await assertNuwaN1BoundedLoop(page, consoleProblems);
  ...
  const unitSelect = creation.getByLabel("故事单元");
  await unitSelect.waitFor();
  assert.ok(await unitSelect.locator("option").count() >= 2, "Creation scope exposes a selectable formal Story Unit.");
  await creation.getByText("已确认事件（至少一项）", { exact: true }).waitFor();
  const createRoot = creation.getByRole("button", { name: "建立主故事版本", exact: true });
  if (await createRoot.count()) {
  ...
  // current project's UI must not show that target when searched.
  const other = await postFixture(`${apiUrl}/__local/story-studio/projects/create`, { title: "另一部隔离作品", folderSlug: "r5-isolated-other-project" });
  await postFixture(`${apiUrl}/__local/story-studio/agent-permissions/activity`, { projectId: other.data.id, actor: "tianyi", action: "read-context", targets: ["only-other-project-target"], targetType: "other-project", authorConfirmed: false, estimatedProviderCost: 0 });
  const currentReceipts = await getFixture(`${apiUrl}/__local/story-studio/agent-permissions?projectId=${encodeURIComponent(fixtureProjectId)}`);
  assert.equal(currentReceipts.data.receipts.some((receipt) => receipt.targets.includes("only-other-project-target")), false, "Operation receipts remain project-isolated before the UI projection.");

  const toolLauncher = page.locator(".dock-tool-launcher");
  if (await toolLauncher.count()) await toolLauncher.click();
  await page.locator('[data-tool-id="engineering-log"]').click();
  const log = page.getByLabel("工程日志连续活动流");
  await log.waitFor();
  assert.equal(await log.getAttribute("data-provider-calls"), "0", "The operation log is an Owner receipt projection and makes no Provider call.");
  await log.getByLabel("操作类型").selectOption("rehearsal-run");
  assert.ok(await log.locator(".engineering-log-stream li").count() >= 1, "N1 start/step/pause/cancel receipts are filterable in the current project.");
  await log.getByRole("button", { name: "查看回执", exact: true }).first().click();
  await log.getByLabel("操作回执详情").waitFor();
  if (r5ContinuousEvidenceDirectory) await page.screenshot({ path: path.join(r5ContinuousEvidenceDirectory, "07-1440-operation-log-receipt.png"), fullPage: false });
...
async function setupNarrativeFixture({ createRoot = true } = {}) {
  ...
  const root = createRoot ? createNarrativeFixtureRoot() : null;
  narrativeFixture = { unit: unit.data, branch: branch.data, workVersionId: root?.identity.workVersionId ?? null };
}

function createNarrativeFixtureRoot() {
...
async function assertMapM2StoryObservation(page, consoleProblems) {
  ...
  assert.ok(canvasBox && canvasBox.y < 260 && canvasBox.height > 280, `The map canvas must remain the first-screen working surface, not fall below a title or summary stack: ${JSON.stringify(canvasBox)}`);
  ...
  await openMapProperties(page,"地点与局部图");
    ...
    assert.ok(currentCanvasBox, `Map layout save for ${location.title} needs the current canvas after its inspector opens.`);
...
async function assertMapM3AuthorExperience(page, consoleProblems) {
  ...
  const capture = async (name) => { if (mapM3EvidenceDirectory) await page.screenshot({ path: path.join(mapM3EvidenceDirectory, name), fullPage: false }); };
  const base = `${apiUrl}/__local/story-studio`;
  const palette = page.locator('.map-authoring-palette');
  await page.setViewportSize({ width: 1440, height: 900 });
  await gotoProduct(page, `${baseUrl}/library?libraryView=map&locale=zh-CN`);
  ...
  const canvas = page.getByLabel("地点示意图画布");
  await canvas.waitFor();
  assert.equal(await page.locator("#map-m2-inspector").count(), 0, "An empty location inspector must not consume the map working width.");
  const directoryToggle = page.locator('[data-panel-toggle="project-directory"]');
  ...
  const nameDrawing = async (selector, name) => {
    await page.getByRole("button", { name: "浏览", exact: true }).click();
    await page.locator(selector).first().focus();
    await page.locator(selector).first().press("Enter");
    const input = page.getByLabel("图示名称");
    await input.fill(name); await input.blur();
    await page.getByRole("status").getByText(/图示名称已保存/u).waitFor();
  };
  await nameDrawing(".map-drawing.is-land", "北湾海岸");
  await nameDrawing(".map-drawing.is-river", "雾河");
  await nameDrawing(".map-drawing.is-mountain", "北岭山脉");
  await nameDrawing(".map-drawing.is-road", "北湾驿道");

  await page.getByRole("button", { name: "地形", exact: true }).click();
  await page.getByLabel("绘图样式").selectOption("forest");
  const box = await canvas.boundingBox();
  assert.ok(box, "MAP-M3 terrain brush needs a measurable canvas.");
  await page.mouse.move(box.x + box.width * .30, box.y + box.height * .62);
  await page.mouse.down();
  await page.mouse.move(box.x + box.width * .48, box.y + box.height * .68, { steps: 18 });
  await page.mouse.move(box.x + box.width * .62, box.y + box.height * .59, { steps: 14 });
  await page.mouse.up();
  await page.getByRole("status").getByText(/连续地形笔触已保存/u).waitFor();
  await page.getByRole("button", { name: "浏览", exact: true }).click();
  await page.locator(".map-drawing.is-forest").last().focus();
  await page.locator(".map-drawing.is-forest").last().press("Enter");
  await page.getByLabel("图示名称").fill("跨郡松林"); await page.getByLabel("图示名称").blur();
  await page.getByRole("status").getByText(/图示名称已保存/u).waitFor();
  await page.getByLabel("图示笔触宽度").fill("14"); await page.getByLabel("图示笔触宽度").blur();
  await page.getByRole("status").getByText(/图示尺寸已保存/u).waitFor();
  await page.getByRole("button", { name: "撤销", exact: true }).click();
  await page.getByRole("status").getByText(/已撤销/u).waitFor();
  ...
  await page.getByRole("status").getByText(/行政结构规则已保存/u).waitFor();
  const boundarySelect = palette.getByLabel("选择行政区域");
  const drawBoundary = async (label, points) => {
    await boundarySelect.selectOption({ label });
    const boundaryBox = await canvas.boundingBox();
    assert.ok(boundaryBox, `The ${label} boundary needs a measurable canvas.`);
    for (const point of points) await canvas.click({ position: { x: boundaryBox.width * point[0], y: boundaryBox.height * point[1] } });
    await page.getByRole("group", { name: "行政边界编辑" }).getByRole("button", { name: "保存边界", exact: true }).click();
    await page.getByRole("status").getByText(/行政边界已保存/u).waitFor();
  };
  await drawBoundary("东郡", [[.48, .18], [.88, .20], [.86, .82], [.51, .78]]);
  await drawBoundary("西郡", [[.12, .20], [.47, .18], [.50, .78], [.15, .84]]);
  await capture("01-north-bay-author-map.png");
  const mapBodyGeometry = await page.locator(".map-workbench-body").evaluate((body) => ({ width: body.getBoundingClientRect().width, canvas: body.querySelector(".map-workbench-canvas")?.getBoundingClientRect().width, inspector: Boolean(body.querySelector("#map-m2-inspector")), overflow: document.documentElement.scrollWidth > window.innerWidth }));
  assert.ok(mapBodyGeometry.canvas > mapBodyGeometry.width * .58 && !mapBodyGeometry.inspector && !mapBodyGeometry.overflow, `The unselected map remains the primary work surface=${JSON.stringify(mapBodyGeometry)}`);

  await fogRow.getByRole("button", { name: "进入局部图", exact: true }).click();
  await page.getByRole("status").getByText(/局部地图和明确入口已保存/u).waitFor();
  await page.getByText("门厅", { exact: true }).waitFor();
  await page.getByText("守卫室", { exact: true }).waitFor();
  await page.getByText("内门", { exact: true }).waitFor();
  ...
  await page.locator(".map-drawing.is-planet").nth(1).focus();
  ...
  assert.match(page.url(), /mapElement=drawing\./u, "The saved answer receipt returns to the exact selected drawing identity.");
  await capture("07-exact-map-source-return.png");

  const visual = await getFixture(`${base}/visual-workbench?projectId=${encodeURIComponent(fixtureProjectId)}`);
  const northBayMap = visual.data.documents.find((document) => document.type === "map" && document.title === "北湾作者地图");
  const fogLocalMap = visual.data.documents.find((document) => document.type === "map" && document.title === "雾港 · 局部地图");
  const starfieldMap = visual.data.documents.find((document) => document.type === "map" && document.title === "北湾星域");
  const planetLocalMap = visual.data.documents.find((document) => document.type === "map" && document.title === "澜星 · 局部地图");
  assert.ok(northBayMap && fogLocalMap && starfieldMap && planetLocalMap, "All four MAP-M3 author maps persist through the existing VisualDocument owner.");
  assert.equal(northBayMap.content.regions.filter((region) => region.layerId === "layer.administration").length, 2, "The author map persists two independently editable administration regions.");
  assert.ok(northBayMap.content.drawings.some((drawing) => drawing.label === "跨郡松林" && drawing.points.length >= 10 && drawing.width === 14), "The continuous forest brush, edit, undo and redo result persists.");
...
async function assertMapM4ManagementAiEditing(page, consoleProblems) {
  ...
  await northBayCard.locator("summary").click();
  ...
  await manager.getByText("可放入当前父图", { exact: true }).waitFor();
  ...
  for (const [side, x, y] of [["子图",10,10],["父图",60,20],["子图",50,10],["父图",60,36]]) {
    const surface=calibrationPanel.getByLabel(side+"对应点选择画布");
    const box=await surface.boundingBox();
    assert.ok(box && box.width>400 && box.height>230,"Each correspondence is picked on a recognizable map, not a tiny preview.");
    await surface.click({position:{x:box.width*x/100,y:box.height*y/100}});
  }
  await calibrationPanel.getByText("高级：精确坐标调整",{exact:true}).click();
  ...
  await page.setViewportSize({ width: 1152, height: 720 });
  const compactCalibrationGeometry = await calibrationPanel.boundingBox();
  const saveGeometry=await calibrationPanel.getByRole("button",{name:"保存定位",exact:true}).boundingBox();
  assert.ok(compactCalibrationGeometry && saveGeometry && saveGeometry.y+saveGeometry.height<=720,"Alignment task actions remain visible at 1152x720.");
  ...
  await calibrationPanel.getByRole("button", { name: "保存定位", exact: true }).click();
  await page.getByRole("status").getByText(/空间定位已保存为一个地图修订/u).waitFor();
  ...
  await reopenedCalibration.getByText("高级：精确坐标调整",{exact:true}).click();
  ...
  assert.equal(await reopenedCalibration.getByRole("button", { name: "保存定位", exact: true }).isDisabled(), true, "Degenerate controls cannot replace the saved calibration.");
  await reopenedCalibration.getByRole("button", { name: "取消", exact: true }).click();
  await page.getByRole("button", { name: /编辑 雾港 · 局部地图 的坐标校准/u }).click();
  await reopenedCalibration.getByText("高级：精确坐标调整",{exact:true}).click();
  ...
  await page.getByRole("button",{name:"预览子图内容",exact:true}).click();
  ...
  const copiedUnlocated = manager.getByText("北湾作者地图 副本", { exact: true }).locator("xpath=parent::section");
  ...
  await spatialEditor.getByText(/父图：北湾作者地图/u).waitFor();
  ...
  const originalVertexCount = await page.getByRole("button", {name:/范围控制点 \d+/u}).count();
  ...
  assert.equal(await page.getByRole("button", {name:/范围控制点 \d+/u}).count(), originalVertexCount, "Reload reopens the saved polygon without rectangularizing it.");
...
async function assertMapAuthorWorkspaceR2(page, consoleProblems) {
  ...
  await openMapMenu(page);
    ...
    await page.getByRole("status").getByText(/地图名称已保存/u).waitFor();
    return (await readMaps()).find(m=>m.title===title);
  };
  const north=await createMap("geography","北湾区域图");
  const fog=await createMap("blank","雾港街区图");
  const stage=page.locator(".map-workbench-stage");
  const clickStage=async(x,y)=>{const b=await stage.boundingBox();assert.ok(b);await page.getByLabel("地点示意图画布").click({position:{x:(b.x-(await page.getByLabel("地点示意图画布").boundingBox()).x)+b.width*x/100,y:(b.y-(await page.getByLabel("地点示意图画布").boundingBox()).y)+b.height*y/100}});};
  const stroke=async(points,name)=>{
    await page.getByRole("button",{name:"线条",exact:true}).click();
    await page.getByLabel("绘图样式").selectOption("road");
    for(const [x,y] of points)await clickStage(x,y);
    await page.getByRole("button",{name:"完成这一笔",exact:true}).click();
    await page.getByRole("status").getByText(/绘图已保存/u).waitFor();
    await page.getByLabel("图示名称").fill(name);await page.getByLabel("图示名称").blur();
    await page.getByRole("status").getByText(/图示名称已保存/u).waitFor();
  };
  await stroke([[20,50],[50,50],[80,50]],"临港街 · 西桥至东码头");
  await stroke([[60,20],[60,50],[60,80]],"海关巷");
  await page.getByRole("button",{name:"标记",exact:true}).click();
  await page.getByLabel("绘图样式").selectOption("building");
  await clickStage(60,35);
  await page.getByRole("status").getByText(/符号已保存/u).waitFor();
  ...
  let box=await space.boundingBox();
  await space.click({position:{x:box.width*.57,y:box.height*.48}});
  const editor=page.getByRole("region",{name:"空间定位编辑器"});
  assert.ok(Math.abs(Number(await editor.getByLabel("定位水平位置").inputValue())-57)<.3);
  await editor.getByRole("button",{name:"改为范围",exact:true}).click();
  await page.getByLabel("空间视口缩放").selectOption("1.5");
  const handle=page.getByRole("button",{name:"范围控制点 1",exact:true});
  const hb=await handle.boundingBox();box=await space.boundingBox();
  await page.mouse.move(hb.x+hb.width/2,hb.y+hb.height/2);await page.mouse.down();await page.mouse.move(hb.x+hb.width/2-30,hb.y+hb.height/2-18,{steps:6});await page.mouse.up();
  ...
  await page.getByRole("status").getByText(/空间定位已保存/u).waitFor();
  const rangeSaved=(await readMaps()).find(m=>m.id===north.id).content.placements.find(p=>p.childMapId===fog.id);
  assert.equal(rangeSaved.bounds.length,4);
  assert.ok(rangeSaved.bounds[0].x<46,"Pointer and keyboard edits move the original vertex under 150% viewport zoom.");
  await page.getByRole("button",{name:"编辑 雾港街区图 的范围定位",exact:true}).press("Enter");
  await editor.getByLabel("定位水平位置").fill("100");
  const edgeVertices=await page.locator(".map-spatial-placement.is-selected polygon").getAttribute("points");
  assert.ok(edgeVertices.split(" ").every(p=>Number(p.split(",")[0])<=100),"Moving to the edge constrains the whole original polygon.");
  await editor.getByRole("button",{name:"取消",exact:true}).click();
  assert.deepEqual((await readMaps()).find(m=>m.id===north.id).content.placements.find(p=>p.childMapId===fog.id),rangeSaved);
  await page.getByRole("button",{name:"编辑 雾港街区图 的范围定位",exact:true}).press("Enter");
  await editor.getByRole("button",{name:"改为坐标校准",exact:true}).click();
  const alignment=page.getByRole("region",{name:"坐标校准"});
  for(const [side,x,y] of [["子图",20,50],["父图",43,52],["子图",80,50],["父图",66,48]]) {
    const canvas=alignment.getByLabel(side+"对应点选择画布");const b=await canvas.boundingBox();
    if(side==="子图"&&x===20) {
      await alignment.getByLabel("对齐视口缩放").selectOption("1.5");
      await page.mouse.move(b.x+b.width/2,b.y+b.height/2);await page.mouse.down({button:"middle"});await page.mouse.move(b.x+b.width/2+15,b.y+b.height/2+10,{steps:4});await page.mouse.up({button:"middle"});
    }
  ...
  await page.getByRole("status").getByText(/空间定位已保存/u).waitFor();
  const saved=(await readMaps()).find(m=>m.id===north.id);
  const cal=saved.content.placements.find(p=>p.childMapId===fog.id);
  const radians=cal.transform.rotation*Math.PI/180;
  const third={x:cal.transform.translateX+cal.transform.scale*(50*Math.cos(radians)-50*Math.sin(radians)),y:cal.transform.translateY+cal.transform.scale*(50*Math.sin(radians)+50*Math.cos(radians))};
  assert.ok(Math.abs(third.x-54.5)<.3&&Math.abs(third.y-50)<.3,"The independent mid-street landmark matches the parent road, not just the two chosen controls.");
  await page.reload();await manager.waitFor();await manager.getByRole("button",{name:"空间总览",exact:true}).click();
  await page.getByLabel("空间总览父地图").selectOption(north.id);
  await page.getByRole("button",{name:"编辑 雾港街区图 的坐标校准",exact:true}).click();
  await alignment.getByRole("button",{name:"5. 叠加预览",exact:true}).click();
  await alignment.getByRole("button",{name:"取消",exact:true}).click();
  ...
  await editor.getByRole("button",{name:"保存定位",exact:true}).click();await page.getByRole("status").getByText(/空间定位已保存/u).waitFor();
  const roomPlaced=manager.locator("aside section").filter({has:page.getByText(room.title,{exact:true})});
  await roomPlaced.getByRole("button",{name:"进入子图",exact:true}).click();
  await page.getByRole("button",{name:"文字",exact:true}).click();await page.getByLabel("标注文字").fill("查验柜台");await clickStage(68,35);
  await page.getByRole("status").getByText(/文字已保存/u).waitFor();
  await capture("r2-05-interior-drawing-1440x900.png");
  await page.setViewportSize({width:1152,height:720});
  const compactCanvas=await page.getByLabel("地点示意图画布").boundingBox();
  assert.ok(compactCanvas.width>400&&compactCanvas.height>300);
  assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true,"Long map titles and task tools do not create page-level horizontal overflow.");
  await capture("r2-06-interior-1152x720.png");
  await page.setViewportSize({width:1440,height:900});
  await page.getByRole("button",{name:"返回父图空间视图",exact:true}).click();
  assert.equal(await page.getByLabel("空间总览父地图").inputValue(),fog.id,"Interior return restores the street parent, not the first map.");
  await page.getByLabel("选择地图",{exact:true}).selectOption(fog.id);
  await page.getByRole("button",{name:"上层",exact:true}).click();
  assert.match(page.url(),new RegExp(encodeURIComponent(north.id)));
  await page.getByLabel("选择地图",{exact:true}).selectOption(fog.id);
  assert.equal(new URL(page.url()).searchParams.get("mapZoom"),fogView,"Reopening the street preserves its author viewport.");
  const raster = await page.evaluate(() => {
    const canvas=document.createElement("canvas");canvas.width=1000;canvas.height=800;
    const c=canvas.getContext("2d");c.fillStyle="#dce9e6";c.fillRect(0,0,1000,800);
    c.fillStyle="#a7c8d2";c.fillRect(0,0,180,800);c.strokeStyle="#8d7350";c.lineWidth=8;
    c.beginPath();c.moveTo(200,400);c.lineTo(800,400);c.stroke();
...
async function assertWorldMaterialsM1(page, consoleProblems) {
  ...
  await page.waitForFunction(async () => {
    const rendered = document.querySelector(".map-workbench-background image");
    if (!(rendered instanceof SVGImageElement)) return false;
    const image = new Image(); image.src = rendered.href.baseVal;
    try { await image.decode(); return image.naturalWidth >= 900 && image.naturalHeight >= 500; } catch { return false; }
  });
  const visual = await getFixture(`${base}/visual-workbench?projectId=${encodeURIComponent(fixtureProjectId)}`);
...
async function assertCharacterObservationDragAndRecovery(page) {
  ...
  try {
    ...
    assert.ok(fiveGeometry && Math.abs(fiveGeometry.trayWidth - fiveGeometry.canvasWidth) <= 1 && Math.abs(fiveGeometry.trayX - fiveGeometry.canvasX) <= 1, `5-person tray must remain aligned with the canvas column=${JSON.stringify({ initialGeometry, fiveGeometry })}`);
    ...
    await reloadProduct(page);
    await page.getByTestId("character-observation-dropzone").getByText("5/5 人", { exact: true }).waitFor();
  ...
  await panel.getByText("本次推演已放弃；既有草稿和历史回执均保留。", { exact: true }).waitFor().catch(async (cause) => {
    const [dom, persisted] = await Promise.all([
      panel.evaluate((element) => ({
        phase: element.getAttribute("data-prediction-phase"),
        view: element.getAttribute("data-prediction-view"),
        runId: element.getAttribute("data-run-id"),
        error: element.querySelector(".tianyi-error")?.textContent?.trim() ?? null,
        text: element.innerText.slice(0, 1_500),
        announcedRun: window.__storyStudioPredictionRun ?? null,
        announcedView: window.__storyStudioPredictionView ?? null
      })),
