...
async function assertNuwaN1BoundedLoop(page, consoleProblems) {
  ...
  await nuwaDirectory.getByRole("tab", { name: "故事结构" }).waitFor();
  await nuwaDirectory.getByRole("button", { name: "雾港追踪", exact: true }).click();
  await nuwaDirectory.getByText(/2 条已保存结果/u).waitFor();
  assert.match(await nuwaDirectory.locator(".nuwa-directory-selection").innerText(), /2 条已保存结果/u, "Browsing a story unit locates saved results without changing the Run scope.");
  ...
  await workspace.getByRole("button", { name: "故事结构", exact: true }).click();
  await page.getByTestId("nuwa-unified-directory").getByRole("tab", { name: "故事结构" }).waitFor();
  await page.getByTestId("nuwa-unified-directory").getByRole("button", { name: "收起目录" }).click();
