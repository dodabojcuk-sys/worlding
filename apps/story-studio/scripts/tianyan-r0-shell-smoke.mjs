...
async function assertNuwaN1BoundedLoop(page, consoleProblems) {
  ...
  await workspace.getByRole("button", { name: "故事结构", exact: true }).click();
  await page.getByTestId("nuwa-unified-directory").getByRole("button", { name: "故事结构 · 选择事件线" }).click();
  await page.waitForURL(/\/event-line/u);
  await page.getByTestId("event-line-journey").waitFor();
  await page.goBack();
  await workspace.waitFor();
  assert.equal(await workspace.getAttribute("data-run-status"), "paused", "Phone story browsing returns to the same paused Run.");
  await workspace.getByRole("button", { name: "排演脉络", exact: true }).click();
  await workspace.locator(".nuwa-n1-auxiliary.is-open.is-drawer").waitFor();
  await workspace.getByRole("button", { name: "排演脉络", exact: true }).click();
  await workspace.getByRole("button", { name: /选择结果.*已选/u }).click();
