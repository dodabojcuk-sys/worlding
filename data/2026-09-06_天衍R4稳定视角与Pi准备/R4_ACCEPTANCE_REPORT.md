...
| 同一 \`480f134\` 的两个远端作业一个全绿、一个只在假 Provider 取消状态超时；目录流程均通过 | 用户点“停止运行”后，已接收的流回执先收敛，取消回执再成为该 Run 的最终 Session 投影 | 流分段与取消可以同时调用现有 Session Owner；竞争写入时取消可能命中内容哈希冲突，浏览器按约不做乐观取消，因此一直看不到“已取消” | 运行时按 Run 串行化唯一 Owner append；新增“流写入尚未完成时取消”的竞态回归，随后晚到 Provider 完成仍被取消终态拒绝；不增加轮询或等待 |
